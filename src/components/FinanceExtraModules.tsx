...
export const FinanceExtraModules: React.FC<FinanceExtraModulesProps> = ({
  ...
  const displayUserName = currentUserName || (currentUser as any)?.displayName || 'Comptable Principal';
  ...
  const handleCreatePaySlip = async () => {
    ...
    const slip = {
      ...
      recordedByName: displayUserName
    };
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
            <div className="bg-white rounded-[32px] p-8 max-w-2xl w-full space-y-6 border border-gray-100 shadow-2xl relative">
              ...
              <div id="payroll-slip-printable" className="p-4 bg-white border border-gray-200 rounded-2xl space-y-6 text-slate-800">
                ...
                <div className="pt-4 border-t border-dashed border-gray-200 flex justify-between items-end text-[9px] text-gray-400 leading-snug">
                  ...
                  <div className="text-center w-48 border-t border-slate-300 pt-1">
                    ...
                    <p className="text-[9px] text-indigo-600 font-bold italic mt-0.5">Émis par : {selectedSlip.recordedByName || displayUserName}</p>
              ...
              <div className="flex justify-end gap-3.5">
                ...
                <button
                  onClick={async () => {
                    await generatePaySlipPDF(selectedSlip, currentEstablishment, selectedSlip.recordedByName || displayUserName);
                  }}
                  className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase text-xs flex items-center gap-1.5 rounded-xl shadow-lg shadow-emerald-100 dark:shadow-none cursor-pointer"
                >
                  <FileText size={15} /> Télécharger en PDF
                </button>
                <button
                  onClick={() => {
                    ...
                    if (printContents) {
                      ...
                      if (printWindow) {
                        printWindow.document.write(`<html><head><title>Bulletin de Paie - ${selectedSlip.employeeName}</title><style>body { font-family: sans-serif; padding: 20px; }</style></head><body>${printContents}</body></html>`);
                  ...
                  className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-750 text-white font-black uppercase text-xs flex items-center gap-1.5 rounded-xl shadow-lg shadow-indigo-100 dark:shadow-none cursor-pointer"
                >
