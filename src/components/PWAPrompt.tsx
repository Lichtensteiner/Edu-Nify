...
export default function PWAPrompt() {
  ...
  return (
    <>
      {/* 1. FLOATING RE-USABLE BANNER CARD */}
      <AnimatePresence>
        {bannerVisible && (
          <motion.div 
            ...
            className="fixed bottom-6 left-4 right-4 md:left-auto md:right-6 md:max-w-md z-[150] drop-shadow-2xl"
          >
            <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-indigo-900 via-indigo-950 to-slate-900 p-5 text-white shadow-2xl border border-indigo-500/20">
              {/* Background radial highlight */}
              <div className="absolute -right-12 -top-12 h-36 w-36 rounded-full bg-indigo-500/10 blur-2xl animate-pulse" />
              <div className="absolute -bottom-12 -left-12 h-36 w-36 rounded-full bg-indigo-500/15 blur-2xl" />
              
              <div className="relative z-10 flex flex-col gap-4">
                <div className="flex items-start gap-3.5">
                  <div className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white p-1.5 border border-white/20 shadow-lg">
                    <img src="/logo.png" alt="Edu-Nify" className="h-full w-full object-contain rounded-lg" referrerPolicy="no-referrer" />
                    <div className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-indigo-600 text-white border-2 border-slate-900 shadow-sm">
                      <Download size={10} className="animate-bounce" />
                    </div>
                  </div>
                  <div className="min-w-0 flex-1">
                    <h3 className="text-sm font-black flex items-center gap-1.5 uppercase tracking-wider text-indigo-200">
                    ...
                    <p className="text-xs font-semibold text-slate-100 mt-1 leading-relaxed">
                      {detectedOS === 'desktop'
                        ? "Exécutez Edu-Nify en plein écran autonome, ultra-rapide et directement depuis votre bureau !"
                        : "Profitez d'une fluidité parfaite et d'un accès instantané depuis votre écran d'accueil sans passer par les Stores !"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-xs font-black text-white hover:bg-indigo-500 hover:shadow-lg transition-all active:scale-95 cursor-pointer uppercase tracking-wider shadow-md shadow-indigo-600/30"
                  >
                    {isInstallable ? <Download size={13} /> : <HelpCircle size={13} />}
                    {isInstallable ? "Installer en 1 Clic" : "Voir le Guide"}
                  </button>
                  <button
                    onClick={handleDismiss}
                    className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/10 text-slate-300 hover:bg-white/20 hover:text-white transition-all cursor-pointer"
                    title="Plus tard"
                  >
                    <X size={16} />
                  </button>
