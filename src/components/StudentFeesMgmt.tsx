    ...
    <div className="space-y-6 font-sans">
      ...
      {receiptPayment && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white text-slate-900 rounded-2xl w-full max-w-md shadow-2xl p-4 sm:p-6 border border-slate-200 relative max-h-[90vh] overflow-y-auto">
            <button 
            ...
            <div id="school-voucher" className="space-y-4 font-sans text-xs">
              <div className="text-center pb-4 border-b border-dashed border-slate-200 mt-2">
                <h2 className="text-xs sm:text-sm font-black uppercase text-indigo-900 break-words">{currentEstablishment?.nom || 'ÉTABLISSEMENT RÈGLEMENTAIRE'}</h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 text-[10px] gap-2 pt-2 text-slate-600">
                <div>
              ...
              <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl space-y-1 text-[10px] text-slate-700">
                ...
                <p className="font-black text-slate-900 uppercase break-words">{receiptPayment.studentName}</p>
                <p className="text-[8.5px]">Etablissement ID : <span className="font-bold">{receiptPayment.etablissement}</span></p>
              </div>

              <div className="pt-2">
                <div className="flex justify-between font-black text-xs text-slate-800 pb-2 border-b border-dashed border-slate-200 gap-2">
                  <span>Nature de l'imputation :</span>
                  <span className="uppercase text-indigo-750 text-right">{getCategoryLabel(receiptPayment.type)}</span>
                </div>
                
                <div className="flex justify-between items-center sm:items-baseline pt-3 pb-3 bg-indigo-50/50 px-3 rounded-xl mt-2 text-slate-900 gap-2">
                  <span className="text-[10px] font-bold text-indigo-900 uppercase">Versement Encaissé :</span>
                  <span className="text-base sm:text-lg font-black text-indigo-900 font-mono text-right">{(receiptPayment.amount || 0).toLocaleString()} FCFA</span>
                </div>
              </div>

              <div className="text-[9.5px] text-slate-500 leading-relaxed border-t border-dashed border-slate-200 pt-3 italic break-words">
                <strong>Notes :</strong> {receiptPayment.notes || 'Règlement de scolarité valide.'}
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 text-[8.5px] gap-3 text-slate-400 pt-3 text-center border-t border-neutral-100">
                <div>
                  <p className="font-extrabold uppercase">Le Guichetier / Caissier</p>
                  <p className="text-slate-900 font-bold mt-1.5 sm:mt-6">{receiptPayment.recordedByName || 'Comptable Principal'}</p>
                </div>
                <div>
                  <p className="font-extrabold uppercase">Signature & Date</p>
                  <p className="text-[8px] font-mono mt-1.5 sm:mt-6">Certifié SAGE 2026</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 pt-4 border-t border-slate-100 flex flex-col sm:flex-row gap-2">
              <button
                onClick={() => setReceiptPayment(null)}
                className="flex-1 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-lg text-[10px] uppercase transition-colors cursor-pointer"
              >
                Fermer
              </button>
              <button
                onClick={async () => {
                  await generateReceiptPDF(receiptPayment, currentEstablishment);
                }}
                className="flex-1 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-lg text-[10px] uppercase transition-all flex items-center justify-center gap-1 shadow-md shadow-emerald-100 cursor-pointer"
              >
                <FileText size={12} /> Télécharger PDF
              </button>
              <button
                onClick={() => {
                  const printContents = document.getElementById('school-voucher')?.innerHTML;
                  if (printContents) {
                    const printWindow = window.open('', '_blank');
                    if (printWindow) {
                      printWindow.document.write(`<html><head><title>Reçu - ${receiptPayment.reference}</title><style>body { font-family: sans-serif; padding: 20px; }</style></head><body>${printContents}</body></html>`);
                ...
                className="flex-1 py-1.5 bg-indigo-600 hover:bg-indigo-750 text-white font-black rounded-lg text-[10px] uppercase transition-all flex items-center justify-center gap-1 shadow-md shadow-indigo-100 cursor-pointer"
              >
