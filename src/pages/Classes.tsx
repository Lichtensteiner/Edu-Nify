...
export default function Classes() {
  ...
  const handleOpenModal = (mode: 'create' | 'edit' | 'view', classItem?: any) => {
    ...
    if (mode === 'edit' && classItem) {
      ...
      setFormData({ nom: '', niveau: systemConfig.niveaux[0] || '', professeur_principal_id: '', enseignants_ids: [], heure_debut: '08:00', matieres: [] });
    }
    setIsModalOpen(true);
    if (mode === 'view') {
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-lg max-h-[92vh] sm:max-h-[88vh] overflow-hidden flex flex-col border border-gray-100 dark:border-gray-750/70 transition-colors duration-200 animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden">
              <div className="p-6 overflow-y-auto space-y-4 flex-1 custom-scrollbar">
                ...
                <div>
                  <label className="block text-[10px] font-black text-gray-450 dark:text-gray-400 uppercase tracking-widest mb-1.5">Niveau / Filière ({currentEstablishment?.systemeScolaire || 'Système Français'})</label>
                  <select
                    value={formData.niveau}
                    onChange={(e) => setFormData({...formData, niveau: e.target.value})}
                    className="w-full px-4 py-2.5 bg-gray-55 dark:bg-gray-900 text-gray-900 dark:text-white border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all text-sm font-semibold"
                  >
                    {systemConfig.niveaux.map((niv) => (
