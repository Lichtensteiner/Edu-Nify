...
export default function Attendance() {
  ...
  const [offlineAttendanceQueue, setOfflineAttendanceQueue] = useState<any[]>(() => {
    try {
  ...
  useEffect(() => {
    localStorage.setItem('attendance_offline_queue', JSON.stringify(offlineAttendanceQueue));
  ...
  const syncOfflineAttendance = async () => {
    if (offlineAttendanceQueue.length === 0) return;
    if (!isOnline) return;

    setSyncing(true);
    let successCount = 0;
    const remainingQueue = [...offlineAttendanceQueue];

    for (const item of offlineAttendanceQueue) {
      try {
        if (item.type === 'update') {
          await updateDoc(doc(db, 'attendance', item.id), item.data);
        }
        successCount++;
        remainingQueue.shift();
      } catch (err) {
        console.error("Error synchronizing attendance item:", err);
        break;
      }
    }

    setOfflineAttendanceQueue(remainingQueue);
    setSyncing(false);
  };

  useEffect(() => {
    if (isOnline && offlineAttendanceQueue.length > 0) {
  ...
  const saveEdit = async (id: string) => {
    ...
    try {
      ...
      if (!isOfflineMode) {
        ...
        // Cache locally in offline queue
        setOfflineAttendanceQueue(prev => [...prev, {
          type: 'update',
          id: id,
          data: updateData
        }]);
    ...
    <div className="space-y-6">
      ...
      <div className={`p-4 rounded-2xl border flex flex-col md:flex-row items-center justify-between gap-4 transition-all ${
          ...
          <div>
            <div className="text-xs font-bold uppercase tracking-wider flex items-center gap-1.5">
              ...
              <span className="underline">
                {(!isOnline) ? 'Mode Hors-ligne Sécurisé (Cache local Gabon-Sync)' : 'Connecté Cloud Gabon-Nify'}
              </span>
            </div>
            <p className="text-[10px] opacity-85 mt-0.5">
              {(!isOnline)
                ? `Les modifications d'appel sont stockées localement. (${offlineAttendanceQueue.length} mise(s) à jour en attente).`
                : "Toutes les présences sont envoyées en temps réel vers la base centrale."
              }
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
          {offlineAttendanceQueue.length > 0 && (
            <button
              onClick={syncOfflineAttendance}
              disabled={syncing || !isOnline}
              className="px-3 py-1 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-300 text-white font-bold text-[10px] uppercase tracking-wider rounded-lg shadow transition-all flex items-center gap-1 cursor-pointer"
            >
              {syncing ? 'Synchro...' : `Synchroniser (${offlineAttendanceQueue.length})`}
            </button>
          )}
        </div>
      </div>
