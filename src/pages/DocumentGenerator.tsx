...
export function drawMiniFlag(doc: jsPDF, code: string, x: number, y: number, w: number, h: number) {
  const previousColor = doc.getFillColor();
  if (code === "GA") {
    doc.setFillColor(76, 175, 80); doc.rect(x, y, w, h / 3, 'F');
    doc.setFillColor(255, 235, 59); doc.rect(x, y + h / 3, w, h / 3, 'F');
    doc.setFillColor(33, 150, 243); doc.rect(x, y + (2 * h) / 3, w, h / 3, 'F');
  } else if (code === "FR") {
    doc.setFillColor(33, 150, 243); doc.rect(x, y, w / 3, h, 'F');
    doc.setFillColor(255, 255, 255); doc.rect(x + w / 3, y, w / 3, h, 'F');
    doc.setFillColor(244, 67, 54); doc.rect(x + (2 * w) / 3, y, w / 3, h, 'F');
  } else if (code === "CA") {
    doc.setFillColor(244, 67, 54); doc.rect(x, y, w / 4, h, 'F');
    doc.setFillColor(255, 255, 255); doc.rect(x + w / 4, y, w / 2, h, 'F');
    doc.setFillColor(244, 67, 54); doc.rect(x + (3 * w) / 4, y, w / 4, h, 'F');
    
    doc.setFillColor(244, 67, 54);
    const cx = x + w / 2;
    const cy = y + h / 2;
    doc.triangle(cx, cy - h/3, cx - w/12, cy, cx + w/12, cy, 'F');
    doc.triangle(cx - w/10, cy - h/6, cx, cy + h/6, cx - w/12, cy + h/3, 'F');
    doc.triangle(cx + w/10, cy - h/6, cx, cy + h/6, cx + w/12, cy + h/3, 'F');
  }
}
...
export default function DocumentGenerator() {
  ...
  useEffect(() => {
    if (currentEstablishment?.id) {
      setSelectedEstablishmentId(currentEstablishment.id);
    }
...
Génère une réponse sous forme d'un objet JSON pur et valide (sans aucun blabla, sans balise markdown de code, juste l'objet brut) contenant strictement ces clés :
{
  ...
  const drawWatermark = async (doc: jsPDF, width: number, height: number) => {
    if (!aiGraphicCharter.showWatermark) return;
    const anyDoc = doc as any;
    const logoUrl = activeEst?.logo || '/logo.png';
    ...
    try {
      ...
      const transparentLogo = await getSemiTransparentLogoDataUrl(imgData, 0.05);
      const wSize = 85;
      anyDoc.saveState();
  ...
  const generateStudentCard = async (student: Student, config: any) => {
    ...
    try {
      ...
      await drawLogo(doc, 5, 4, 10);
      
      drawMiniFlag(doc, headerInfo.codePays, 17, 3.8, 3.5, 2.2);
  ...
  const generateCertificate = async (student: Student, config: any) => {
    ...
    try {
      ...
      await drawWatermark(doc, 210, 297);
      drawHeaderDecoration(doc, 210);

      // Get official system-specific header
      const headerInfo = getSystemHeaderInfo(activeEst?.systemeScolaire, activeEst);
      const isGabon = headerInfo.pays.includes("GABON");
      const isFrance = headerInfo.pays.includes("FRAN");

      if (isGabon) {
        doc.setFillColor(76, 175, 80); doc.rect(0, 0, 70, 3, 'F'); // Green
        doc.setFillColor(255, 235, 59); doc.rect(70, 0, 70, 3, 'F'); // Yellow
        doc.setFillColor(33, 150, 243); doc.rect(140, 0, 70, 3, 'F'); // Blue
      } else if (isFrance) {
        doc.setFillColor(33, 150, 243); doc.rect(0, 0, 70, 3, 'F'); // Blue
        doc.setFillColor(255, 255, 255); doc.rect(70, 0, 70, 3, 'F'); // White
        doc.setFillColor(244, 67, 54); doc.rect(140, 0, 70, 3, 'F'); // Red
      } else {
        doc.setFillColor(primaryColorRGB[0], primaryColorRGB[1], primaryColorRGB[2]); doc.rect(0, 0, 210, 3, 'F');
      }

      await drawLogo(doc, 15, 12, 22);
      
      // Center country flag above name
  ...
  const generateReportCard = async (student: Student, config: any) => {
    ...
    try {
      ...
      if (isGabon) {
        doc.setFillColor(76, 175, 80); doc.rect(8, 8, 64, 1.5, 'F'); // Green
        doc.setFillColor(255, 235, 59); doc.rect(72, 8, 64, 1.5, 'F'); // Yellow
        doc.setFillColor(33, 150, 243); doc.rect(136, 8, 66, 1.5, 'F'); // Blue
      } else if (isFrance) {
        doc.setFillColor(33, 150, 243); doc.rect(8, 8, 64, 1.5, 'F'); // Blue
        doc.setFillColor(255, 255, 255); doc.rect(72, 8, 64, 1.5, 'F'); // White
        doc.setFillColor(244, 67, 54); doc.rect(136, 8, 66, 1.5, 'F'); // Red
      }

      doc.setFillColor(secondaryColorRGB[0], secondaryColorRGB[1], secondaryColorRGB[2]);
      doc.rect(8, 50, 194, 1.5, 'F');
      
      try {
        await drawLogo(doc, 15, 14, 20);
      } catch (e) {
        console.warn("Skipping logo in PDF due to error");
      }
      
      // Center country flag above title inside bulletin dark header
      drawMiniFlag(doc, headerInfo.codePays, 111, 9.5, 8, 4);

      doc.setTextColor(255);
      doc.setFontSize(12);
      doc.setFont(aiGraphicCharter.titleFont || "times", "bold");
      doc.text("BULLETIN TRIMESTRIEL DE NOTES", 115, 18.5, { align: 'center' });
      
      doc.setFontSize(7);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(241, 245, 249);
      doc.text(`${headerInfo.pays} | « ${headerInfo.deviseNationale} »`, 115, 23.5, { align: 'center' });
      doc.text(headerInfo.ministere, 115, 27.5, { align: 'center' });
      doc.text(headerInfo.direction, 115, 31.5, { align: 'center' });
      
      doc.setFontSize(9.5);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(255, 255, 255);
      doc.text(headerInfo.nomEtablissement.toUpperCase(), 115, 36.5, { align: 'center' });
      
      doc.setFontSize(7);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(241, 245, 249);
      doc.text(`${headerInfo.adresseEtablissement} | ${headerInfo.contactEtablissement}`, 115, 40.5, { align: 'center' });
      doc.text(`${config.academicYear || "ANNÉE ACADÉMIQUE 2025-2026"} | ${config.period?.toUpperCase() || "TRIMESTRE 1"}`, 115, 44.5, { align: 'center' });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col gap-4">
          ...
          <div className="space-y-2 max-h-[140px] overflow-y-auto pr-1">
            {establishments.filter(est => est.id === currentEstablishment?.id).map((est) => (
              <button
                key={est.id}
