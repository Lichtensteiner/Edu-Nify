...
export default function Settings() {
  ...
  const [activeTab, setActiveTab] = useState<'appearance' | 'profile' | 'notifications' | 'security' | 'system' | 'customization'>('appearance');
  
  const { currentEstablishment, updateEstablishment } = useEstablishment();
  const [custSysteme, setCustSysteme] = useState(currentEstablishment?.systemeScolaire || 'Système Français');
  const [custNameChef, setCustNameChef] = useState(currentEstablishment?.systemeCustomNameChef || 'Proviseur');
  const [custPeriodes, setCustPeriodes] = useState(currentEstablishment?.systemeCustomPeriodes || 'Trimestres');
  const [custNotes, setCustNotes] = useState(currentEstablishment?.systemeCustomNotes || 'Moyenne sur 20');
  const [custExamens, setCustExamens] = useState<string[]>(currentEstablishment?.systemeCustomExamens || []);
  const [custVacances, setCustVacances] = useState(currentEstablishment?.systemeCustomVacances || 'Calendrier Standard');
  const [custNiveaux, setCustNiveaux] = useState(currentEstablishment?.systemeCustomNiveaux || 'Classes');
  const [custBiometrieObligatoire, setCustBiometrieObligatoire] = useState(currentEstablishment?.biometrieObligatoire !== false);
  const [savingCust, setSavingCust] = useState(false);

  useEffect(() => {
    if (currentEstablishment) {
      setCustSysteme(currentEstablishment.systemeScolaire || 'Système Français');
      setCustNameChef(currentEstablishment.systemeCustomNameChef || 'Proviseur');
      setCustPeriodes(currentEstablishment.systemeCustomPeriodes || 'Trimestres');
      setCustNotes(currentEstablishment.systemeCustomNotes || 'Moyenne sur 20');
      setCustExamens(currentEstablishment.systemeCustomExamens || []);
      setCustVacances(currentEstablishment.systemeCustomVacances || 'Calendrier Standard');
      setCustNiveaux(currentEstablishment.systemeCustomNiveaux || 'Classes');
      setCustBiometrieObligatoire(currentEstablishment.biometrieObligatoire !== false);
    }
  ...
  const [geminiKey, setGeminiKey] = useState(() => {
    try {
      ...
      // Write the default key so it is active permanently
      localStorage.setItem('GEMINI_API_KEY', 'AQ.Ab8RN6JNgmskAvlSxyPhK-0tBRWDvmsQ2w_263iUGZneAdAOHg');
      return 'AQ.Ab8RN6JNgmskAvlSxyPhK-0tBRWDvmsQ2w_263iUGZneAdAOHg';
    } catch {
      return 'AQ.Ab8RN6JNgmskAvlSxyPhK-0tBRWDvmsQ2w_263iUGZneAdAOHg';
    }
  });
  ...
  const [smtpTestEmail, setSmtpTestEmail] = useState(currentUser?.email || '');
  const [smtpTestResult, setSmtpTestResult] = useState<{ status: 'idle' | 'success' | 'error'; message: string }>({ status: 'idle', message: '' });

  const handleTestSMTP = async () => {
    if (!smtpTestEmail) {
      notifyError("Veuillez saisir une adresse e-mail de test.");
      return;
    }
    setTestingSMTP(true);
    ...
    try {
      ...
      if (response.ok && data.success) {
        setSmtpTestResult({
          status: 'success',
          message: data.message || `E-mail de test envoyé avec succès à ${smtpTestEmail}!`
        });
        notifySuccess("Connexion SMTP validée ! E-mail de test envoyé.");
        addLog(`Test SMTP réussi vers ${smtpTestEmail}`, "system");
        confetti();
      } else {
        throw new Error(data.error || "Une erreur est survenue lors du test SMTP.");
      }
    } catch (err: any) {
      console.error(err);
      setSmtpTestResult({
        status: 'error',
        message: err.message || "Impossible de se connecter au serveur SMTP."
      });
      notifyError("Échec du test SMTP.");
      addLog(`Échec du test SMTP vers ${smtpTestEmail}`, "system");
    } finally {
  ...
  const togglePreference = async (key: keyof typeof notifPreferences) => {
    ...
    const newPrefs = { ...notifPreferences, [key]: nextVal };
    ...
    try {
      ...
      // Real-time visual toast notifications
      if (key === 'push') {
        if (nextVal) {
          notifySuccess("Notifications Push activées en temps réel sur cet appareil !");
        } else {
          notifyInfo("Notifications Push désactivées.");
        }
      } else if (key === 'email') {
        if (nextVal) {
          notifySuccess("Email Hebdomadaire activé ! Résumés des notes et activités activés.");
        } else {
          notifyInfo("Email Hebdomadaire désactivé.");
        }
      } else if (key === 'sms') {
        if (nextVal) {
          notifySuccess("Alertes SMS activées ! Urgences et retards en temps réel.");
        } else {
          notifyInfo("Alertes SMS désactivées.");
        }
      } else if (key === 'urgent') {
        if (nextVal) {
          notifySuccess("Alertes Système critiques activées pour les maintenances.");
        } else {
          notifyInfo("Alertes Système critiques désactivées.");
        }
      }
    } catch (error) {
      console.error('Error updating notifications:', error);
      setNotifPreferences(prev => ({ ...prev, [key]: !prev[key] })); // Rollback
      notifyError("Échec de la mise à jour des préférences de notification.");
    }
  };
  ...
  const tabs = [
    ...
    ...(currentUser?.role === 'admin' || currentUser?.preciseRole?.includes('Super') || currentUser?.email === 'martinienmvezogo@gmail.com'
      ? [{ id: 'customization', label: 'Personnalisation', icon: Sliders }]
      : [])
  ];
            ...
            >
              ...
              {activeTab === 'system' && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  {/* Configuration & Test du Serveur SMTP */}
                  {isAdmin && (
                    <div className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 dark:from-emerald-950/20 dark:to-emerald-900/10 p-6 sm:p-8 rounded-3xl border border-emerald-100/80 dark:border-emerald-900/50 space-y-6 relative overflow-hidden">
                      <div className="absolute top-0 right-0 p-6 opacity-5 sm:opacity-10 pointer-events-none">
                        <Globe size={120} className="text-emerald-600 dark:text-emerald-400" />
                      </div>
                      
                      <div className="relative z-10 max-w-3xl space-y-4">
                        <div>
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-emerald-600 text-white shadow-sm mb-3">
                            ⚡ Notification SMTP Temps Réel
                          </span>
                          <h4 className="text-xl font-black text-gray-900 dark:text-white">Configuration & Test du Serveur de Messagerie SMTP</h4>
                          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 font-medium leading-relaxed">
                            L'écosystème d'Edu-Nify envoie des notifications d'accès en temps réel aux nouveaux utilisateurs (élèves, parents, personnel) via SMTP. 
                            Le serveur utilise les secrets d'environnement configurés de manière sécurisée en production.
                          </p>
                        </div>

                        <div className="space-y-4">
                          <div className="p-4 bg-white/60 dark:bg-gray-900/40 rounded-2xl border border-emerald-100 dark:border-emerald-900/30 grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                              <span className="text-[9px] font-bold uppercase text-gray-400 tracking-wider">HÔTE DE CONNEXION</span>
                              <p className="text-sm font-bold text-gray-700 dark:text-gray-300">smtp.gmail.com</p>
                            </div>
                            <div>
                              <span className="text-[9px] font-bold uppercase text-gray-400 tracking-wider">EXPÉDITEUR CONFIGURÉ</span>
                              <p className="text-sm font-bold text-gray-700 dark:text-gray-300">ludo.consulting3@gmail.com</p>
                            </div>
                          </div>

                          <div className="space-y-2">
                            <label className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest pl-1">Tester la distribution temps réel (Saisissez un e-mail)</label>
                            <div className="relative flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                              <input 
                                ...
                                className="flex-1 pl-4 pr-4 py-3.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl text-sm font-medium focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all shadow-sm"
                              />
                              <button
                                type="button"
                                onClick={handleTestSMTP}
                                disabled={testingSMTP}
                                className="px-6 py-3.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white font-bold text-sm rounded-2xl transition-all shadow-md shadow-emerald-100 dark:shadow-none flex items-center justify-center gap-2 shrink-0 cursor-pointer"
                              >
                                {testingSMTP ? (
                            ...
                            <motion.div 
                              ...
                              className={`p-4 rounded-2xl border flex items-start gap-3 text-xs font-bold ${
                                smtpTestResult.status === 'success' 
                                  ? 'border-emerald-100 bg-emerald-50 text-emerald-800 dark:border-emerald-900/30 dark:bg-emerald-950/10 dark:text-emerald-400' 
                                  : 'border-rose-100 bg-rose-50 text-rose-800 dark:border-rose-900/30 dark:bg-rose-950/10 dark:text-rose-400'
                              }`}
                            >
                              <div className="mt-0.5 shrink-0">
                                {smtpTestResult.status === 'success' ? (
                              ...
                              <div>
                                <p className="font-extrabold uppercase text-[10px] tracking-wider mb-0.5">
                                  {smtpTestResult.status === 'success' ? 'Validation SMTP réussie ✓' : 'Échec de connexion SMTP'}
                                </p>
                                <p className="font-medium leading-relaxed">{smtpTestResult.message}</p>
                              </div>
                            </motion.div>
                          </div>
                        )}
                ...
                <div className="space-y-8 font-sans">
                  <div>
                    <h3 className="text-2xl font-black text-gray-900 dark:text-white flex items-center gap-3">
                      ...
                      Personnalisation par Système Éducatif
                    </h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-medium mt-1">
                      Adaptez le vocabulaire, le système d'évaluation, les examens d'État et le calendrier de la plateforme aux spécificités de votre établissement.
                    </p>
                  </div>

                  {/* Warning Box */}
                  <div className="p-4 bg-indigo-50/50 dark:bg-indigo-950/10 border border-indigo-100 dark:border-indigo-900/30 rounded-2xl flex items-start gap-3">
                    <Building2 className="text-indigo-600 dark:text-indigo-400 shrink-0 mt-0.5" size={16} />
                    <div className="text-xs text-indigo-900 dark:text-indigo-300 font-medium leading-relaxed">
                      <span className="font-bold">Branding Établissement Actif :</span> Vous personnalisez actuellement les paramètres de l'établissement <span className="font-black underline">{currentEstablishment?.nom || 'Ludo_Consulting'}</span>. Toutes les modifications s'appliqueront immédiatement à l'ensemble des bulletins, tableaux de bord et notifications des élèves et parents.
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {/* Left Column: Core System Select & Fast Templates */}
                    <div className="space-y-6">
                      <div className="flex flex-col gap-2">
                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                          🇨🇦 🇫🇷 🇬🇦 Système Scolaire Référent
                        </label>
                        <select
                          value={custSysteme}
                          onChange={(e) => setCustSysteme(e.target.value)}
                          className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-bold text-indigo-600 dark:text-indigo-400 focus:ring-2 focus:ring-indigo-500 outline-none"
                        >
                          <option value="Système Français">🇫🇷 Système Français (AEFE / Conventionné)</option>
                          <option value="Système Franco-Canadien">🇨🇦 Système Franco-Canadien</option>
                          <option value="Système Public Gabonais">🇬🇦 Système Public Gabonais (Ministère Éducation Nationale)</option>
                          <option value="Système Privé Gabonais">🇬🇦 Système Privé Gabonais</option>
                          <option value="École Conventionnée">🇬🇦 École Conventionnée (Gabon)</option>
                          <option value="École Bilingue">🇬🇧 École Bilingue (International / Anglo-Saxon)</option>
                        </select>
                      </div>

                      {/* Fast Apply Template Button */}
                      <div className="p-5 bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/20 dark:to-purple-950/10 border border-indigo-100/50 dark:border-indigo-900/30 rounded-2xl space-y-3">
                        <h4 className="text-xs font-black text-indigo-800 dark:text-indigo-400 uppercase tracking-wider flex items-center gap-1.5">
                          <Award size={14} /> Préconfiguration Rapide
                        </h4>
                        <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed font-medium">
                          Remplissez automatiquement tous les champs terminologiques avec les standards officiels de ce système scolaire en un clic.
                        </p>
                        <button
                          type="button"
                          onClick={() => {
                            if (custSysteme === "Système Français") {
                              setCustNameChef("Proviseur");
                              setCustPeriodes("Trimestres");
                              setCustNotes("Moyenne sur 20");
                              setCustNiveaux("Classes");
                              setCustVacances("Calendrier Scolaire Français (AEFE)");
                              setCustExamens(["Baccalauréat Français", "Brevet des Collèges"]);
                            } else if (custSysteme === "Système Franco-Canadien") {
                              setCustNameChef("Directeur d'école");
                              setCustPeriodes("Semestres");
                              setCustNotes("GPA (sur 4.0)");
                              setCustNiveaux("Niveaux");
                              setCustVacances("Calendrier Canadien (Semaine de relâche)");
                              setCustExamens(["Diplôme d'Études Secondaires (DES)"]);
                            } else if (custSysteme === "Système Public Gabonais") {
                              setCustNameChef("Proviseur");
                              setCustPeriodes("Trimestres");
                              setCustNotes("Moyenne sur 20");
                              setCustNiveaux("Classes");
                              setCustVacances("Calendrier Officiel Éducation Nationale Gabonaise");
                              setCustExamens(["Baccalauréat Gabonais", "BEPC", "CEPE"]);
                            } else if (custSysteme === "Système Privé Gabonais") {
                              setCustNameChef("Directeur Général");
                              setCustPeriodes("Trimestres");
                              setCustNotes("Moyenne sur 20");
                              setCustNiveaux("Classes");
                              setCustVacances("Calendrier National Gabonais");
                              setCustExamens(["Baccalauréat Gabonais", "BEPC"]);
                            } else if (custSysteme === "École Conventionnée") {
                              setCustNameChef("Proviseur");
                              setCustPeriodes("Trimestres");
                              setCustNotes("Moyenne sur 20");
                              setCustNiveaux("Classes");
                              setCustVacances("Calendrier AEFE / Mixte Gabonais");
                              setCustExamens(["Baccalauréat Gabonais", "Baccalauréat Français", "BEPC"]);
                            } else if (custSysteme === "École Bilingue") {
                              setCustNameChef("Headmaster");
                              setCustPeriodes("Terms");
                              setCustNotes("Grade Point Average (GPA)");
                              setCustNiveaux("Grades");
                              setCustVacances("Calendrier International Anglo-Saxon");
                              setCustExamens(["Cambridge IGCSE", "IELTS / TOEFL", "International Baccalaureate (IB)"]);
                            }
                            notifySuccess(`Modèle de configuration "${custSysteme}" appliqué avec succès ! N'oubliez pas d'enregistrer.`);
                          }}
                          className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-black text-xs transition-colors shadow-sm uppercase tracking-wider cursor-pointer"
                        >
                          Appliquer le modèle standard
                        </button>
                      </div>

                      {/* Customization Details */}
                      <div className="space-y-4">
                        <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest border-b pb-2">Vocabulaire & Terminologie</h4>
                        
                        <div className="flex flex-col gap-1.5">
                          <label className="text-xs font-bold text-gray-600 dark:text-gray-300">Appellation du Chef d'Établissement</label>
                          <input 
                            type="text"
                            value={custNameChef}
                            onChange={(e) => setCustNameChef(e.target.value)}
                            placeholder="ex: Proviseur, Principal, Directeur..."
                            className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-extrabold outline-none"
                          />
                          <div className="flex gap-2 flex-wrap">
                            {["Proviseur", "Directeur Général", "Principal", "Headmaster", "Directeur d'école"].map(term => (
                              <button 
                                key={term}
                                type="button"
                                onClick={() => setCustNameChef(term)}
                        ...
                        <div className="flex flex-col gap-1.5 mt-3">
                          <label className="text-xs font-bold text-gray-600 dark:text-gray-300">Découpage des Périodes Scolaires</label>
                          <input 
                            type="text"
                            value={custPeriodes}
                            onChange={(e) => setCustPeriodes(e.target.value)}
                            placeholder="ex: Trimestres, Semestres, Sessions..."
                            className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-extrabold outline-none"
                          />
                          <div className="flex gap-2 flex-wrap">
                            {["Trimestres", "Semestres", "Sessions", "Terms"].map(term => (
                              <button 
                                key={term}
                                type="button"
                                onClick={() => setCustPeriodes(term)}
                                className={`px-2 py-0.5 text-[10px] rounded-md font-bold border transition-all ${
                                  custPeriodes === term 
                                    ? 'bg-indigo-50 border-indigo-200 text-indigo-600 dark:bg-indigo-950/20 dark:border-indigo-800 dark:text-indigo-400' 
                                    : 'bg-white border-gray-200 text-gray-400 dark:bg-gray-800 dark:border-gray-700'
                        ...
                        <div className="flex flex-col gap-1.5 mt-3">
                          <label className="text-xs font-bold text-gray-600 dark:text-gray-300">Appellation des Niveaux scolaires</label>
                          <input 
                            type="text"
                            value={custNiveaux}
                            onChange={(e) => setCustNiveaux(e.target.value)}
                            placeholder="ex: Classes, Grades, Sections, Niveaux..."
                            className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-extrabold outline-none"
                          />
                          <div className="flex gap-2 flex-wrap">
                            {["Classes", "Grades", "Sections", "Niveaux"].map(term => (
                              <button 
                                key={term}
                                type="button"
                                onClick={() => setCustNiveaux(term)}
                                className={`px-2 py-0.5 text-[10px] rounded-md font-bold border transition-all ${
                                  custNiveaux === term 
                                    ? 'bg-indigo-50 border-indigo-200 text-indigo-600 dark:bg-indigo-950/20 dark:border-indigo-800 dark:text-indigo-400' 
                                    : 'bg-white border-gray-200 text-gray-400 dark:bg-gray-800 dark:border-gray-700'
                    ...
                    {/* Right Column: Grading policy, state exams, calendar */}
                    <div className="space-y-6">
                      <div className="space-y-4">
                        <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest border-b pb-2">Politique d'Évaluation & Notes</h4>
                        
                        <div className="flex flex-col gap-1.5">
                          <label className="text-xs font-bold text-gray-600 dark:text-gray-300">Système de notation par défaut</label>
                          <select
                            value={custNotes}
                            onChange={(e) => setCustNotes(e.target.value)}
                            className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-extrabold outline-none"
                          >
                            <option value="Moyenne sur 20">🇫🇷 / 🇬🇦 Échelle classique : Moyenne sur 20 points</option>
                            <option value="GPA (sur 4.0)">🇨🇦 / 🇺🇸 Échelle Nord-Américaine : GPA (sur 4.0)</option>
                            <option value="Lettres (A-F)">🇬🇧 Échelle Internationale : Lettres (A, B, C, D, E, F)</option>
                            <option value="Pourcentage (%)">📈 Pourcentage brut : de 0% à 100%</option>
                          </select>
                        </div>

                        <div className="p-4 bg-amber-50/50 dark:bg-amber-950/10 border border-amber-100 dark:border-amber-900/30 rounded-2xl flex items-start gap-3 mt-4">
                          <div className="pt-0.5">
                            <input
                              id="toggle-biometrics-mandatory"
                              type="checkbox"
                              checked={custBiometrieObligatoire}
                              onChange={(e) => setCustBiometrieObligatoire(e.target.checked)}
                              className="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 cursor-pointer"
                            />
                          </div>
                          <div>
                            <label htmlFor="toggle-biometrics-mandatory" className="text-xs font-bold text-gray-900 dark:text-gray-100 flex items-center gap-1.5 cursor-pointer">
                              🔑 Biométrie obligatoire au premier démarrage
                            </label>
                            <p className="text-[10px] text-gray-500 dark:text-gray-400 leading-relaxed mt-1">
                              <strong>Activé (Par défaut) :</strong> Bloque l'accès à l'application pour les élèves/profs tant qu'ils n'ont pas fait de scan.<br />
                              <strong>Désactivé (Recommandé) :</strong> Rend la biométrie optionnelle. Les élèves peuvent contourner via le bouton « Passer cette étape », idéal si l'établissement n'est pas encore équipé en scanners dédiés.
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest border-b pb-2">Préparation aux Examens Nationaux & d'État</h4>
                        <div className="p-4 bg-gray-50 dark:bg-gray-900/40 rounded-2xl border border-gray-100 dark:border-gray-800 space-y-3">
                          <p className="text-xs font-medium text-gray-400">Cochez les examens préparés dans votre établissement au Gabon :</p>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                            {[
                              "Baccalauréat Gabonais",
                              ...
                              const checked = custExamens.includes(exam);
                              return (
                                <label 
                                  key={exam} 
                                  className="flex items-center gap-2.5 p-2 bg-white dark:bg-gray-850 border border-gray-200/50 dark:border-gray-800 rounded-xl cursor-pointer text-xs font-bold text-gray-800 dark:text-gray-300 hover:border-indigo-400 select-none transition-colors"
                                >
                                  <input 
                                    type="checkbox"
                                    checked={checked}
                                    onChange={(e) => {
                                      if (e.target.checked) {
                                        setCustExamens(prev => [...prev, exam]);
                                      } else {
                                        setCustExamens(prev => prev.filter(x => x !== exam));
                                      }
                                    }}
                      ...
                      <div className="space-y-4">
                        <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest border-b pb-2">Calendrier des Vacances / Congés scolaires</h4>
                        <div className="flex flex-col gap-1.5">
                          <label className="text-xs font-bold text-gray-600 dark:text-gray-300">Modèle de Calendrier</label>
                          <input 
                            type="text"
                            value={custVacances}
                            onChange={(e) => setCustVacances(e.target.value)}
                            placeholder="ex: Calendrier Éducation Nationale Gabonaise..."
                            className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-extrabold outline-none"
                          />
                          <div className="flex gap-2 flex-wrap mt-1">
                            {[
                              "Calendrier Gabonais Officiel", 
                              "Calendrier AEFE (Système Français)", 
                              "Calendrier Canadien", 
                              "Calendrier International Anglo-Saxon"
                            ].map(term => (
                              <button 
                                key={term}
                                type="button"
                                onClick={() => setCustVacances(term)}
                                className={`px-2 py-0.5 text-[10px] rounded-md font-bold border transition-all ${
                                  custVacances === term 
                                    ? 'bg-indigo-50 border-indigo-200 text-indigo-600 dark:bg-indigo-950/20 dark:border-indigo-800 dark:text-indigo-400' 
                                    : 'bg-white border-gray-200 text-gray-400 dark:bg-gray-800 dark:border-gray-700'
                  ...
                  {/* Gabon High-School Series specifically for Gabonese schools */}
                  {(custSysteme.includes("Gabonais") || custSysteme === "École Conventionnée") && (
                    <motion.div 
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="p-5 bg-amber-50/50 dark:bg-amber-950/10 border border-amber-100/50 dark:border-amber-900/30 rounded-2xl space-y-4"
                    >
                      <h4 className="text-xs font-black text-amber-800 dark:text-amber-400 uppercase tracking-widest flex items-center gap-1.5">
                        🇬🇦 Spécificités du Système Secondaire Gabonais (Séries)
                      </h4>
                      <p className="text-xs text-gray-500 dark:text-gray-400 font-medium leading-relaxed">
                        Le système de l'enseignement secondaire gabonais s'organise en séries spécifiques au Lycée (Séries Littéraires, Scientifiques, Techniques et Tertiaires). Elles déterminent les coefficients pour le Baccalauréat Gabonais.
                      </p>
                      <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-3 mt-2">
                        {[
                          { name: "Série A1", desc: "Littéraire (Latin/Grec)" },
                          { name: "Série A2", desc: "Littéraire (Langues)" },
                          { name: "Série B", desc: "Économique & Sociale" },
                          { name: "Série C", desc: "Sciences Physiques/Maths" },
                          { name: "Série D", desc: "Sciences de la vie/Maths" },
                          { name: "Série E", desc: "Mathématiques & Tech" },
                          { name: "Série G", desc: "Tertiaire / Gestion" },
                          { name: "Série TI", desc: "Technologies de l'Info" }
                        ].map(serie => (
                          <div 
                            key={serie.name} 
                            title={serie.desc}
                            className="p-3 bg-white dark:bg-gray-900 border border-amber-200/50 dark:border-amber-900/30 rounded-xl text-center shadow-xs"
                          >
                            <span className="block font-black text-amber-700 dark:text-amber-400 text-xs">{serie.name}</span>
                            <span className="block text-[8px] font-bold text-gray-400 dark:text-gray-500 mt-0.5 truncate uppercase">{serie.desc.split(" ")[0]}</span>
                          </div>
                        ))}
                      </div>
                    </motion.div>
                  ...
                  {/* Terminology Simulator Preview */}
                  <div className="p-6 bg-gray-50 dark:bg-gray-900/30 border border-gray-150 dark:border-gray-800 rounded-2xl space-y-4">
                    <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                      👁️ Aperçu de Personnalisation en Temps Réel
                    </h4>
                    <p className="text-xs text-gray-400 font-medium">
                      Voici comment vos termes personnalisés s'afficheront sur l'interface et les bulletins :
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div className="p-4 bg-white dark:bg-gray-850 rounded-xl border border-gray-100 dark:border-gray-800 shadow-xs">
                        <span className="text-[9px] font-black text-indigo-500 uppercase tracking-wider block">Signature Bulletin</span>
                        <span className="block font-black text-gray-950 dark:text-white mt-1">Le {custNameChef || 'Proviseur'}</span>
                        <p className="text-[10px] text-gray-400 mt-0.5 font-medium">Le bulletin affichera l'approbation du chef d'établissement sous cette appellation.</p>
                      </div>
                      <div className="p-4 bg-white dark:bg-gray-850 rounded-xl border border-gray-100 dark:border-gray-800 shadow-xs">
                        <span className="text-[9px] font-black text-indigo-500 uppercase tracking-wider block">Trimestre / Semestre</span>
                        <span className="block font-black text-gray-950 dark:text-white mt-1">1er {custPeriodes ? custPeriodes.replace(/s$/, '') : 'Trimestre'}</span>
                        <p className="text-[10px] text-gray-400 mt-0.5 font-medium">Les moyennes de notes seront regroupées sous des périodes appelées ainsi.</p>
                      </div>
                      <div className="p-4 bg-white dark:bg-gray-850 rounded-xl border border-gray-100 dark:border-gray-800 shadow-xs">
                        <span className="text-[9px] font-black text-indigo-500 uppercase tracking-wider block">Notation Moyenne</span>
                        <span className="block font-black text-emerald-600 mt-1">16.50 / {(custNotes?.includes("20") ? "20" : custNotes?.includes("4") ? "4.0 (GPA)" : "Standard")}</span>
                        <p className="text-[10px] text-gray-400 mt-0.5 font-medium">Le calcul des rapports et des classements sera structuré selon ce barème.</p>
                      </div>
                    </div>
                  </div>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        if (!currentEstablishment?.id) {
                          notifyError("Aucun établissement actif sélectionné.");
                          return;
                        }
                        setSavingCust(true);
                        try {
                          await updateEstablishment(currentEstablishment.id, {
                            systemeScolaire: custSysteme,
                            systemeCustomNameChef: custNameChef,
                            systemeCustomPeriodes: custPeriodes,
                            systemeCustomNotes: custNotes,
                            systemeCustomExamens: custExamens,
                            systemeCustomVacances: custVacances,
                            systemeCustomNiveaux: custNiveaux,
                            biometrieObligatoire: custBiometrieObligatoire
                          });
                          notifySuccess("Félicitations ! Les paramètres de votre système éducatif ont été personnalisés et sauvegardés avec succès !");
                          confetti({
                            particleCount: 80,
                            spread: 60,
                            origin: { y: 0.8 },
                            colors: ['#4f46e5', '#ea580c', '#ffffff']
                          });
                          addLog("Système éducatif personnalisé avec succès", "system");
                        } catch (err) {
                          console.error(err);
                          notifyError("Impossible de sauvegarder la personnalisation.");
                        } finally {
                          setSavingCust(false);
                        }
                      }}
                      className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-bold text-xs rounded-xl shadow-md transition-all flex items-center gap-1.5 uppercase tracking-wider cursor-pointer"
                    >
                      {savingCust ? (
                      ...
                      {savingCust ? "Enregistrement..." : "Enregistrer la personnalisation"}
                    </button>
                  </div>
                </div>
              )}
