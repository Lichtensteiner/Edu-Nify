...
import { formatGrade } from '../constants/educationalSystems';
import { 
  collection, 
  query, 
  where, 
...
const Grades: React.FC = () => {
  ...
  const [newGrade, setNewGrade] = useState({
    ...
    comment: '',
    classId: '',
    type: 'evaluation' as 'interrogation' | 'evaluation',
    period: 'Trimestre 1',
  ...
  const handleOpenAddModal = () => {
    ...
    setNewGrade({
      ...
      classId: '',
      type: 'interrogation',
      period: selectedPeriodBulletin || 'Trimestre 1',
      changeReason: ''
    });
  ...
  const handleOpenEditModal = (grade: Grade) => {
    ...
    setNewGrade({
      ...
      period: grade.period || 'Trimestre 1',
      changeReason: ''
    });
    setShowAddModal(true);
  };
    ...
    try {
      ...
      if (isOfflineMode) {
        ...
        const offlineItem: Grade = {
          ...
          period: newGrade.period || 'Trimestre 1'
        };
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <form id="gradeForm" onSubmit={handleSaveGrade} className="space-y-4 text-left">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1">Type d'Évaluation</label>
                      <select
                        required
                        value={newGrade.type}
                        ...
                        className="w-full px-3.5 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-750 rounded-xl outline-none text-xs cursor-pointer font-bold text-gray-700 dark:text-gray-200"
                      >
                        <option value="evaluation">📝 Devoir / Évaluation</option>
                        <option value="interrogation">⚡ Interrogation / Quiz</option>
                      </select>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[9px] text-indigo-500 font-bold block mt-1">
                        💡 Auto ({currentEstablishment?.systemeScolaire || 'Français'})
                      </span>
                    </div>
                  </div>
