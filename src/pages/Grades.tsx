...
import { 
  ...
  formatGrade, 
  getGradeMention, 
  validateGradeAntiFraud, 
  SystemConfig,
  SystemHonorsTier,
  EDUCATIONAL_SYSTEMS_CONFIG 
} from '../constants/educationalSystems';
import { 
  collection, 
  query, 
  where, 
...
const Grades: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'grades' | 'anomalies' | 'bulletins' | 'audit'>('grades');
  
  // Active Educational System Configuration
  const systemName = currentEstablishment?.systemeScolaire || 'Système Français';
  const systemConfig: SystemConfig = useMemo(() => getSystemConfig(systemName), [systemName]);
  ...
  const [selectedPeriodBulletin, setSelectedPeriodBulletin] = useState<string>(systemConfig.periods[0] || 'Trimestre 1');
  ...
  const [bulletinForm, setBulletinForm] = useState({
    studentId: '',
    period: systemConfig.periods[0] || 'Trimestre 1',
    remark: '',
    councilDecision: '',
    absencesCount: 0,
    latenessesCount: 0,
  ...
  // Interactive Anti-Fraud Simulator State
  const [simScore, setSimScore] = useState<string>(systemConfig.defaultMaxScore.toString());
  const [simMaxScore, setSimMaxScore] = useState<string>(systemConfig.defaultMaxScore.toString());
  const [simPrevScore, setSimPrevScore] = useState<string>((systemConfig.defaultMaxScore * 0.5).toString());
  const [simCoef, setSimCoef] = useState<string>('2');
  const [simResult, setSimResult] = useState<any>(null);

  // Evaluate simulator live
  useEffect(() => {
    const s = parseFloat(simScore);
    const m = parseFloat(simMaxScore) || systemConfig.defaultMaxScore;
    const p = parseFloat(simPrevScore);
    const c = parseFloat(simCoef) || 1;

    if (!isNaN(s)) {
      const res = validateGradeAntiFraud({
        score: s,
        maxScore: m,
        systemName,
        previousScores: isNaN(p) ? [] : [p],
        inputCoefficient: c
      });
      const mention = getGradeMention(s, m, systemName);
      setSimResult({ ...res, mention, formatted: formatGrade(s, systemName, m) });
    } else {
      setSimResult(null);
    }
  }, [simScore, simMaxScore, simPrevScore, simCoef, systemName, systemConfig]);
  ...
  const [newGrade, setNewGrade] = useState({
    ...
    title: '',
    score: '',
    maxScore: systemConfig.defaultMaxScore.toString(),
    coefficient: '1',
    comment: '',
    classId: '',
    type: systemConfig.evaluationTypes[0]?.id || 'ds',
    period: systemConfig.periods[0] || 'Trimestre 1',
    changeReason: '' // required if updating locked
  });

  // Real-time Anti-Fraud validation warning inside form
  const [antiFraudWarning, setAntiFraudWarning] = useState<{ isSuspicious: boolean; flags: string[]; alertLevel: 'OK' | 'WARNING' | 'CRITICAL' }>({
    isSuspicious: false,
    flags: [],
    alertLevel: 'OK'
  });

  // Live validation on newGrade changes
  useEffect(() => {
    if (!newGrade.score || isNaN(parseFloat(newGrade.score))) {
      setAntiFraudWarning({ isSuspicious: false, flags: [], alertLevel: 'OK' });
      return;
    }

    const parsedScore = parseFloat(newGrade.score);
    const parsedMax = parseFloat(newGrade.maxScore) || systemConfig.defaultMaxScore;
    const parsedCoef = parseFloat(newGrade.coefficient) || 1;

    // Filter previous student scores in this subject for statistical anomaly detection
    const prevScores = grades
      .filter(g => g.studentId === newGrade.studentId && g.subject === newGrade.subject && (!editingGrade || g.id !== editingGrade.id))
      .map(g => (g.score / (g.maxScore || 20)) * (parsedMax || 20));

    const check = validateGradeAntiFraud({
      score: parsedScore,
      maxScore: parsedMax,
      systemName,
      previousScores: prevScores,
      inputCoefficient: parsedCoef
    });

    setAntiFraudWarning(check);
  }, [newGrade.score, newGrade.maxScore, newGrade.coefficient, newGrade.studentId, newGrade.subject, systemName, systemConfig, grades, editingGrade]);

  // Real-time automatic coefficient recalculation state machine
  const [lastAutoTrigger, setLastAutoTrigger] = useState({ subject: '', classId: '', type: '' });

  useEffect(() => {
    const system = currentEstablishment?.systemeScolaire || 'Système Français';
    const hasTriggerChanged = 
      newGrade.subject !== lastAutoTrigger.subject || 
      newGrade.classId !== lastAutoTrigger.classId || 
      newGrade.type !== lastAutoTrigger.type;

    if (hasTriggerChanged && newGrade.subject && newGrade.classId) {
      const autoCoef = getAutomaticCoefficient({
        systemeScolaire: system,
        subject: newGrade.subject,
        className: newGrade.classId,
        evaluationType: newGrade.type
      });
      
      setNewGrade(prev => ({
        ...prev,
        coefficient: autoCoef.toString()
      }));
  ...
  }, [newGrade.subject, newGrade.classId, newGrade.type, currentEstablishment?.systemeScolaire]);
  ...
  // Real-time listener for classes and students of the active establishment
  useEffect(() => {
    // 1. Listen to classes strictly for active establishment
    const unsubClasses = onSnapshot(collection(db, 'classes'), (snap) => {
      const clData = snap.docs
        .filter(doc => (doc.data().etablissement || 'EDU-001') === activeEstId && !doc.data().deleted)
        .map(doc => ({ id: doc.id, ...doc.data() }));

      if (clData.length === 0 && activeEstId === 'EDU-001') {
        // Fallback demo classes only for primary default campus
        setClasses([
      ...
      console.error("Error loading classroom defaults:", error);
    });

    // 2. Listen to students strictly for active establishment
    const unsubUsers = onSnapshot(collection(db, 'users'), (snap) => {
      const stData = snap.docs
        .filter(doc => {
          const data = doc.data();
          const isStudentRole = data.role === 'élève' || data.role === 'eleve';
          const matchesEst = (data.etablissement || 'EDU-001') === activeEstId;
          return isStudentRole && matchesEst;
        })
        .map(doc => ({ id: doc.id, ...doc.data() }));

      if (stData.length === 0 && activeEstId === 'EDU-001') {
        // Fallback demo students only for primary default campus
        setStudents([
      ...
      console.error("Error loading students for establishment:", error);
    });
        ...
        setSafetyLogs([
          {
            ...
            severity: 'INFO',
            details: 'Liaison temps réel établie avec la base de données Firestore à Libreville (Gabon)'
          }
        ]);
      } else {
  ...
  const filteredGrades = grades.filter(grade => {
    ...
    const matchesPeriod = selectedPeriodFilter === 'all' || 
                          grade.period === selectedPeriodFilter || 
                          (!grade.period && selectedPeriodFilter === 'Trimestre 1');
  ...
  const handleOpenAddModal = () => {
    ...
    setNewGrade({
      studentId: '',
      subject: dbSubjects[0] || 'Mathématiques',
      title: '',
      score: '',
      maxScore: systemConfig.defaultMaxScore.toString(),
      coefficient: '1',
      comment: '',
      classId: classes[0]?.nom || classes[0]?.id || '',
      type: systemConfig.evaluationTypes[0]?.id || 'ds',
      period: selectedPeriodFilter !== 'all' ? selectedPeriodFilter : (systemConfig.periods[0] || 'Trimestre 1'),
      changeReason: ''
    });
    setAntiFraudWarning({ isSuspicious: false, flags: [], alertLevel: 'OK' });
  ...
  const handleOpenEditModal = (grade: Grade) => {
    ...
    setNewGrade({
      ...
      maxScore: (grade.maxScore || systemConfig.defaultMaxScore).toString(),
      coefficient: grade.coefficient.toString(),
      comment: grade.comment || '',
      classId: grade.classId,
      type: grade.type || systemConfig.evaluationTypes[0]?.id || 'ds',
      period: grade.period || systemConfig.periods[0] || 'Trimestre 1',
      changeReason: ''
    });
    setShowAddModal(true);
  };
  ...
  const handleSaveGrade = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🛡️ Perform Educational System Anti-Fraud Inspection
      const studentPreviousScores = grades
        .filter(g => g.studentId === newGrade.studentId && g.subject === newGrade.subject && (!editingGrade || g.id !== editingGrade.id))
        .map(g => (g.score / (g.maxScore || 20)) * (parsedMax || 20));

      const validation = validateGradeAntiFraud({
        score: parsedScore,
        maxScore: parsedMax,
        systemName,
        previousScores: studentPreviousScores,
        inputCoefficient: parsedCoef
      });

      let isScoreSuspicious = validation.isSuspicious;
      const suspiciousReasons = [...validation.flags];

      if (editingGrade) {
        const gap = Math.abs(parsedScore - editingGrade.score);
        if (gap >= systemConfig.antiFraudRules.varianceThreshold) {
          isScoreSuspicious = true;
          suspiciousReasons.push(`Écart de modification anormal : ${gap.toFixed(1)} pts (Seuil système : ±${systemConfig.antiFraudRules.varianceThreshold} pts)`);
        }
      }

      const lockDurationMs = (systemConfig.antiFraudRules.lockDelayHours || 48) * 60 * 60 * 1000;
      ...
      if (isOfflineMode) {
        ...
        const offlineItem: Grade = {
          ...
          type: newGrade.type,
          status: 'EN_ATTENTE_VALIDATION',
          lockDeadline: new Date(Date.now() + lockDurationMs),
          isSuspicious: isScoreSuspicious,
          suspiciousReasons: suspiciousReasons,
          period: newGrade.period || systemConfig.periods[0] || 'Trimestre 1'
        };
      ...
      const payload: Omit<Grade, 'id'> & { etablissement: string } = {
        studentId: newGrade.studentId,
        studentName: resolvedStudentNameStr,
        classId: matchedClass?.nom || newGrade.classId || '6ème A',
        subject: newGrade.subject || dbSubjects[0] || 'Mathématiques',
        title: newGrade.title,
        score: parsedScore,
        maxScore: parsedMax,
        coefficient: parsedCoef,
        ...
        lockDeadline: editingGrade ? Timestamp.fromDate(editingGrade.lockDeadline) : Timestamp.fromDate(new Date(Date.now() + lockDurationMs)),
      ...
      if (editingGrade) {
        ...
        if (isScoreSuspicious) {
          ...
          const safetyAlert: SafetyAuditLog = {
            ...
            event: 'CHAPERON_ANTI_FRAUDE_ALERTE',
            severity: validation.alertLevel === 'CRITICAL' ? 'CRITICAL' : 'WARNING',
            details: `Alerte Chaperon (${systemConfig.name}) : ${suspiciousReasons.join(' ; ')} sur ${payload.studentName} par ${payload.teacherName}`
          };
          setSafetyLogs(prev => [safetyAlert, ...prev]);
          notifyError(`🚨 Chaperon Anti-Fraude : ${suspiciousReasons[0] || 'Anomalie suspecte détectée'} ! Audit historisé.`);
        } else {
        ...
        if (isScoreSuspicious) {
          ...
          const safetyAlert: SafetyAuditLog = {
            ...
            actorName: `${currentUser.prenom} ${currentUser.nom}`,
            actorRole: currentUser.role || 'enseignant',
            event: 'CHAPERON_NOUVELLE_NOTE_SUSPECTE',
            severity: validation.alertLevel === 'CRITICAL' ? 'CRITICAL' : 'WARNING',
            details: `Note ajoutée avec alertes chaperon (${systemConfig.name}) : ${suspiciousReasons.join(' ; ')}`
          };
          setSafetyLogs(prev => [safetyAlert, ...prev]);
          notifyError(`⚠️ Note enregistrée mais signalée par le Chaperon Anti-Fraude (${suspiciousReasons[0]}).`);
        } else {
          notifySuccess(`Note scolaire enregistrée ! Verrouillage automatique dans ${systemConfig.antiFraudRules.lockDelayHours}h.`);
        }
      }
  ...
  const flushOfflineQueue = async () => {
    ...
    try {
      ...
      notifySuccess(`⚡ Synchronisation réussie : ${localSyncQueue.length} notes de la file d'attente du Gabon envoyées à Firestore !`);
      setLocalSyncQueue([]);
    } catch (e) {
  ...
  const handleGenerateBulletinData = (studentId: string, targetPeriod?: string) => {
    if (!studentId) return;
    const student = students.find(s => s.id === studentId);
    if (!student) return;

    const periodToUse = targetPeriod || bulletinForm.period || selectedPeriodBulletin || 'Trimestre 1';

    // Filter student grades by period
    const studentGrades = grades.filter(g => 
      g.studentId === studentId && 
      (g.period === periodToUse || (!g.period && periodToUse === 'Trimestre 1'))
    );
    ...
    const mentionInfo = getGradeMention(generalAvg, 20, systemName);
    ...
    setBulletinForm({
      ...
      remark: `${mentionInfo.badge} ${mentionInfo.label} — ${mentionInfo.appreciation}`,
      councilDecision: generalAvg >= systemConfig.passingThreshold ? (mentionInfo.isAdmitted ? 'Admis(e) avec Félicitations' : 'Admis(e)') : 'Avertissement de Travail',
      absencesCount: 0,
      latenessesCount: 0,
      isPublished: true
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
            Gouvernance académique conforme au <strong>{systemConfig.name}</strong> avec intégrité algorithmique, verrouillage différé ({systemConfig.antiFraudRules.lockDelayHours}h) et traçabilité immuable.
          </p>
        </div>

        {/* Navigation tabs */}
        <div className="flex flex-wrap bg-gray-100 dark:bg-gray-950 p-1.5 rounded-2xl border border-gray-200/50 dark:border-gray-850 animate-fade-in gap-1">
          <button
          ...
          >
            <ShieldAlert size={14} className={grades.filter(g => g.isSuspicious).length > 0 ? "text-amber-500 animate-pulse" : "text-emerald-500"} />
            🛡️ Chaperon Anti-Fraude
            {grades.filter(g => g.isSuspicious).length > 0 && (
              <span className="px-1.5 py-0.2 bg-rose-500 text-white rounded-full text-[10px] font-black">
                {grades.filter(g => g.isSuspicious).length}
              </span>
            )}
          </button>
          <button
          ...
          >
            <History size={13} />
            Audit ({historyLogs.length + safetyLogs.length})
          </button>
        </div>
      </div>

      {/* 🏛️ Active Educational System Ribbon Banner */}
      <div className="bg-gradient-to-r from-indigo-900 via-slate-900 to-indigo-950 text-white p-5 rounded-3xl border border-indigo-800/40 shadow-md flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="w-12 h-12 rounded-2xl bg-white/10 border border-white/15 flex items-center justify-center text-2xl shrink-0 shadow-inner">
            {systemConfig.flag}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-indigo-300">Système Éducatif Réglementaire de l'Établissement</span>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 lg:pt-0">
          <div className="bg-white/10 backdrop-blur-sm px-3 py-1.5 rounded-xl border border-white/10 text-center">
            <span className="text-[9px] uppercase tracking-wider text-indigo-200 block font-bold">Échelle de notation</span>
            <span className="text-xs font-black text-white">{systemConfig.evaluationScale}</span>
          </div>

          <div className="bg-white/10 backdrop-blur-sm px-3 py-1.5 rounded-xl border border-white/10 text-center">
            <span className="text-[9px] uppercase tracking-wider text-indigo-200 block font-bold">Seuil d'admission</span>
            <span className="text-xs font-black text-emerald-300">≥ {systemConfig.passingThreshold} / {systemConfig.defaultMaxScore}</span>
          </div>

          <div className="bg-white/10 backdrop-blur-sm px-3 py-1.5 rounded-xl border border-white/10 text-center">
            <span className="text-[9px] uppercase tracking-wider text-indigo-200 block font-bold">Délai Anti-Fraude</span>
            <span className="text-xs font-black text-amber-300">{systemConfig.antiFraudRules.lockDelayHours}h Auto-Lock</span>
          </div>

          <button
            onClick={() => setShowSystemModal(true)}
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-black uppercase tracking-wider transition-all shadow-md flex items-center gap-1.5 cursor-pointer ml-auto lg:ml-0"
          >
            <BookOpen size={14} />
            Barème & Mentions
          </button>
            ...
            >
              ...
              <div>
                ...
                <span className="text-xl font-extrabold text-gray-800 dark:text-white mt-0.5">
                  {formatGrade(getGeneralAverage(), currentEstablishment?.systemeScolaire)}
                </span>
              </div>
            </motion.div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
              ...
              <div className="relative">
                <select
                  ...
                  className="pl-3 pr-8 py-2 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl text-xs outline-none cursor-pointer appearance-none font-semibold text-gray-700 dark:text-gray-200"
                >
                        ...
                        <tr key={grade.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-900/10 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex flex-col">
                              ...
                              <div className="flex items-center gap-1.5 mt-0.5">
                                <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-extrabold uppercase">{grade.subject}</span>
                                <span className="text-[9px] px-1.5 py-0.5 bg-indigo-50/70 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 rounded-md font-extrabold uppercase tracking-wider">
                                  {grade.period || 'Trimestre 1'}
                                </span>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-center">
                            <span className={`text-sm font-black px-2.5 py-1 rounded-lg ${
                              ...
                              {formatGrade((grade.score / grade.maxScore) * 20, currentEstablishment?.systemeScolaire)}
                            </span>
                          </td>
      ...
      {/* TAB: CHAPERON ANTI-FRAUDE & CONFORMITÉ */}
      {activeTab === 'anomalies' && (
        <div className="space-y-6">
          {/* Chaperon Overview Dashboard Grid */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white dark:bg-gray-800 p-5 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-xs font-extrabold uppercase text-gray-400">Conformité Système</span>
              </div>
              <div className="text-2xl font-black text-gray-900 dark:text-white">
                {grades.length > 0 ? `${Math.round(((grades.length - grades.filter(g => g.isSuspicious).length) / grades.length) * 100)}%` : '100%'}
              </div>
              <span className="text-[10.5px] text-emerald-600 font-bold block mt-1">Conforme au {systemConfig.name}</span>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-5 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-xs font-extrabold uppercase text-gray-400">Verrouillage Auto</span>
              </div>
              <div className="text-2xl font-black text-indigo-600 dark:text-indigo-400">
                {systemConfig.antiFraudRules.lockDelayHours} Heures
              </div>
              <span className="text-[10.5px] text-gray-500 font-bold block mt-1">Clôture administrative stricte</span>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-5 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-xs font-extrabold uppercase text-gray-400">Seuil d'Écart Max</span>
              </div>
              <div className="text-2xl font-black text-amber-600">
                ±{systemConfig.antiFraudRules.varianceThreshold} Pts
              </div>
              <span className="text-[10.5px] text-gray-500 font-bold block mt-1">Détection de saut statistique</span>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-5 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-xs font-extrabold uppercase text-gray-400">Alertes Actives</span>
              </div>
              <div className="text-2xl font-black text-rose-600">
                {grades.filter(g => g.isSuspicious).length} Note(s)
              </div>
              <span className="text-[10.5px] text-rose-500 font-bold block mt-1">Nécessite audit ou justification</span>
            </div>
          </div>

          {/* Interactive Anti-Fraud Proctor Live Simulator */}
          <div className="bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-950 text-white p-6 rounded-3xl border border-indigo-900/60 shadow-xl space-y-5">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-indigo-800/40 pb-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base font-black tracking-tight text-white uppercase">
                    Simulateur & Testeur d'Intégrité en Temps Réel
                  </h3>
                </div>
                <p className="text-xs text-indigo-200/80 mt-0.5">
                  Testez instantanément une note ou un coefficient contre l'algorithme du Chaperon Anti-Fraude de <strong>{systemConfig.name}</strong>.
                </p>
              </div>
              <span className="px-3 py-1 bg-indigo-500/20 text-indigo-300 border border-indigo-400/30 rounded-xl text-xs font-black self-start md:self-auto">
                {systemConfig.flag} {systemConfig.evaluationScale}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-800/80 border border-indigo-500/30 rounded-xl outline-none text-white text-sm font-black focus:border-indigo-400"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="number"
                  value={simMaxScore}
                  onChange={(e) => setSimMaxScore(e.target.value)}
                  className="w-full px-3.5 py-2.5 bg-slate-800/80 border border-indigo-500/30 rounded-xl outline-none text-white text-sm font-black focus:border-indigo-400"
                />
              </div>
            ...
            {simResult && (
              <div className="bg-slate-800/50 p-5 rounded-2xl border border-indigo-500/20 grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                <div>
                  <span className="text-[10px] uppercase tracking-wider text-indigo-300 font-extrabold block">Verdict du Chaperon</span>
                  <div className="flex items-center gap-2 mt-1">
                    {simResult.isSuspicious ? (
                      <span className="px-3 py-1 rounded-xl bg-rose-500/20 text-rose-300 border border-rose-500/40 text-xs font-black flex items-center gap-1.5">
                        <AlertTriangle size={14} /> ANOMALIE DÉTECTÉE ({simResult.alertLevel})
                      </span>
                    ) : (
                      <span className="px-3 py-1 rounded-xl bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-xs font-black flex items-center gap-1.5">
                        <CheckCircle2 size={14} /> CONFORME & VALIDE
                      </span>
                    )}
                  </div>
                  {simResult.flags.length > 0 && (
                    <ul className="mt-2 space-y-1 text-[11px] text-rose-300">
                      {simResult.flags.map((flag: string, idx: number) => (
                        <li key={idx}>• {flag}</li>
                      ))}
                    </ul>
                  )}
                ...
                <div>
                  <span className="text-[10px] uppercase tracking-wider text-indigo-300 font-extrabold block">Mention & Barème Système</span>
                  <div className="mt-1 flex items-center gap-2">
                    <span className="text-base font-black text-white">{simResult.formatted}</span>
                    <span className="px-2 py-0.5 rounded-lg bg-indigo-500/20 text-amber-300 border border-amber-400/30 text-xs font-bold">
                      {simResult.mention.badge} {simResult.mention.label}
                    </span>
                  </div>
                  <span className="text-[10px] text-indigo-200/70 block mt-0.5">{simResult.mention.appreciation}</span>
                </div>

                <div>
                  <span className="text-[10px] uppercase tracking-wider text-indigo-300 font-extrabold block">Règle de Clôture</span>
                  <p className="text-xs text-indigo-100 font-bold mt-1">
                    Verrouillage automatique sous {systemConfig.antiFraudRules.lockDelayHours} heures.
                  </p>
                  <span className="text-[10px] text-indigo-300/70 block mt-0.5">
                    Modifications après clôture exigent autorisation de la direction.
                  </span>
                </div>
              </div>
            )}
          ...
          {/* Table of Flagged Suspicious Notes & Discrepancies */}
          <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
              <div>
                <h3 className="text-base font-black text-gray-900 dark:text-white flex items-center gap-2">
                  <ShieldAlert className="text-rose-600" size={18} />
                  Registre des Détections & Anomalies d'Évaluation
                </h3>
                <p className="text-xs text-gray-450 mt-0.5">
                  Liste des notes signalées par l'algorithme d'intégrité ou soumises à un arbitrage exceptionnel.
                </p>
              </div>
              <span className="px-3 py-1 rounded-xl text-xs font-black bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900/30">
                {grades.filter(g => g.isSuspicious).length} Note(s) Signalée(s)
              </span>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700 text-[10px] font-black uppercase text-gray-400 tracking-wider">
                    <th className="px-6 py-3.5">Élève & Classe</th>
                    <th className="px-6 py-3.5">Matière & Enseignant</th>
                    <th className="px-6 py-3.5 text-center">Score</th>
                    <th className="px-6 py-3.5">Motif du Signalement</th>
                    <th className="px-6 py-3.5 text-right">Actions d'Arbitrage</th>
                  </tr>
                </thead>
                    ...
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-gray-400">
                        <div className="flex flex-col items-center justify-center gap-2">
                          ...
                          <span className="font-extrabold text-sm text-gray-700 dark:text-gray-300">Aucune anomalie détectée</span>
                          <span className="text-xs text-gray-450">Toutes les notes saisies respectent scrupuleusement les barèmes du {systemConfig.name}.</span>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    grades.filter(g => g.isSuspicious).map(grade => {
                      return (
                        <tr key={grade.id} className="hover:bg-rose-50/20 dark:hover:bg-rose-950/10">
                          <td className="px-6 py-4">
                            <span className="font-black text-gray-900 dark:text-white block">{grade.studentName}</span>
                            <span className="text-[10px] text-gray-400 font-bold">{grade.classId}</span>
                          </td>

                          <td className="px-6 py-4">
                            <span className="font-bold text-indigo-600 dark:text-indigo-400 block">{grade.subject}</span>
                            <span className="text-[10px] text-gray-500 font-medium">{grade.teacherName || 'Enseignant non spécifié'}</span>
                          </td>

                          <td className="px-6 py-4 text-center">
                            <span className="font-black text-rose-600 bg-rose-50 dark:bg-rose-950/30 px-2.5 py-1 rounded-lg">
                              {formatGrade(grade.score, systemName, grade.maxScore)}
                            </span>
                          </td>

                          <td className="px-6 py-4">
                            <div className="space-y-1">
                              {grade.suspiciousReasons && grade.suspiciousReasons.length > 0 ? (
                                grade.suspiciousReasons.map((reason, i) => (
                                  <span key={i} className="inline-block px-2 py-0.5 rounded bg-rose-100 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 text-[10.5px] font-bold">
                                    ⚠️ {reason}
                                  </span>
                                ))
                              ) : (
                                <span className="text-[10.5px] text-rose-600 font-bold">Écart statistique suspect supérieur au seuil (±{systemConfig.antiFraudRules.varianceThreshold} pts)</span>
                              )}
                            </div>
                          </td>
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              {isAdmin && (
                                <button
                                  onClick={() => handleRequestUnlock(grade)}
                                  className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-black transition-all flex items-center gap-1 cursor-pointer"
                                >
                                  <Unlock size={12} /> Arbitrer
                                </button>
                              )}
                              <button
                                onClick={() => handleOpenEditModal(grade)}
                                className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl text-xs font-bold transition-all cursor-pointer"
                              >
                                Examiner
                              </button>
                            </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <form id="gradeForm" onSubmit={handleSaveGrade} className="space-y-4 text-left">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1">Période / Trimestre</label>
                      <select
                        required
                        value={newGrade.period}
                        onChange={(e) => setNewGrade({ ...newGrade, period: e.target.value })}
                        className="w-full px-3.5 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-750 rounded-xl outline-none text-xs cursor-pointer font-bold text-gray-700 dark:text-gray-200"
                      >
                        {systemConfig.periods.map(period => (
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1">Type d'Évaluation ({systemConfig.name})</label>
                      <select
                        required
                        value={newGrade.type}
                        onChange={(e) => {
                          const val = e.target.value;
                          const matchedType = systemConfig.evaluationTypes.find(t => t.id === val);
                          setNewGrade({
                            ...newGrade,
                            type: val as any,
                            coefficient: matchedType ? matchedType.defaultCoef.toString() : newGrade.coefficient
                          });
                        }}
                        className="w-full px-3.5 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-750 rounded-xl outline-none text-xs cursor-pointer font-bold text-gray-700 dark:text-gray-200"
                      >
                        {systemConfig.evaluationTypes.map(t => (
                          <option key={t.id} value={t.id}>{t.label} (Coef {t.defaultCoef})</option>
                        ))}
                      </select>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1">Saisie Note ({systemConfig.evaluationScale})</label>
                      <input
                        type="number"
                        step="0.25"
                        min="0"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3.5 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-750 rounded-xl outline-none text-xs font-bold text-indigo-600 dark:text-indigo-400"
                      />
                      <span className="text-[9px] text-indigo-500 font-bold block mt-1">
                        💡 Barème {systemConfig.name}
                      </span>
                    </div>
                  </div>

                  {/* 🛡️ Real-time Anti-Fraud & Mention Feedback Box */}
                  {newGrade.score !== '' && !isNaN(parseFloat(newGrade.score)) && (
                    <div className={`p-3.5 rounded-2xl border transition-all ${
                      antiFraudWarning.isSuspicious
                        ? 'bg-rose-50/80 dark:bg-rose-950/20 border-rose-200 dark:border-rose-900/40 text-rose-800 dark:text-rose-300'
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-black">
                            {antiFraudWarning.isSuspicious
                              ? `Alerte Chaperon (${antiFraudWarning.alertLevel})`
                              : 'Conforme au système éducatif'}
                          </span>
                        </div>

                        {/* Mention badge */}
                        {(() => {
                          const parsed = parseFloat(newGrade.score);
                          const parsedMax = parseFloat(newGrade.maxScore) || 20;
                          const m = getGradeMention(parsed, parsedMax, systemName);
                          return (
                            <span className="px-2 py-0.5 rounded-md bg-white/80 dark:bg-gray-900/80 text-[10.5px] font-black shadow-xs">
                              {m.badge} {m.label}
                            </span>
                          );
                        })()}
                      ...
                      {antiFraudWarning.flags.length > 0 && (
                        <ul className="mt-2 space-y-0.5 text-[10.5px] font-medium">
                          {antiFraudWarning.flags.map((flag, idx) => (
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (bulletinForm.studentId) {
                          ...
                          handleGenerateBulletinData(bulletinForm.studentId, newPeriod);
                        }
                      }}
                      required
            ...
            >
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-850 border-t border-gray-150 dark:border-gray-800/80 flex justify-end gap-2.5">
                <button
                  type="button"
                  onClick={async () => {
                    await generateReportCardPDF(viewingBulletin, currentEstablishment);
                  }}
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-xl text-xs uppercase flex items-center gap-1.5 cursor-pointer"
                >
                  <FileText size={14} /> Télécharger en PDF
                </button>
                <button
                  type="button"
                  onClick={() => setViewingBulletin(null)}
      ...
      {/* MODAL : RÉFÉRENTIEL DU SYSTÈME ÉDUCATIF & BARÈMES */}
      <AnimatePresence>
        {showSystemModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div
            ...
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="relative bg-white dark:bg-gray-850 rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]"
              onClick={e => e.stopPropagation()}
            >
              {/* Header */}
              <div className="p-6 bg-gradient-to-r from-slate-900 to-indigo-950 text-white flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="text-3xl">{systemConfig.flag}</div>
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-indigo-300">
                      Référentiel Académique Officiel
                    </span>
              ...
              {/* Body */}
              <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar flex-1 space-y-6 text-left">
                {/* Description */}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="p-3.5 bg-gray-50 dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
                    <span className="text-[9.5px] uppercase font-black text-gray-400 block">Échelle de Notation</span>
                    <span className="text-base font-black text-indigo-600 dark:text-indigo-400 mt-0.5 block">{systemConfig.evaluationScale}</span>
                  </div>
                  <div className="p-3.5 bg-gray-50 dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
                    <span className="text-[9.5px] uppercase font-black text-gray-400 block">Seuil d'Admission</span>
                    <span className="text-base font-black text-emerald-600 dark:text-emerald-400 mt-0.5 block">{systemConfig.passingThreshold}/20</span>
                  </div>
                  <div className="p-3.5 bg-gray-50 dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
                    <span className="text-[9.5px] uppercase font-black text-gray-400 block">Délai Clôture Notes</span>
                    <span className="text-base font-black text-amber-600 mt-0.5 block">{systemConfig.antiFraudRules.lockDelayHours}h</span>
                  </div>
                  <div className="p-3.5 bg-gray-50 dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
                    <span className="text-[9.5px] uppercase font-black text-gray-400 block">Seuil Écart Fraude</span>
                    <span className="text-base font-black text-rose-600 mt-0.5 block">±{systemConfig.antiFraudRules.varianceThreshold} pts</span>
                  </div>
                </div>

                {/* Honors Scale Table */}
                <div className="space-y-2">
                  <h4 className="text-xs font-black uppercase text-gray-700 dark:text-gray-200 flex items-center gap-1.5">
                    <Award size={15} className="text-amber-500" />
                    Barème des Mentions & Distinctions
                  </h4>
                  <div className="border border-gray-150 dark:border-gray-800 rounded-2xl overflow-hidden shadow-xs">
                    <table className="w-full text-xs text-left border-collapse">
                      <thead>
                        <tr className="bg-gray-50 dark:bg-gray-800/60 text-gray-400 font-black uppercase border-b border-gray-150 dark:border-gray-800 text-[10px]">
                          <th className="p-3">Score Requis ({systemConfig.evaluationScale})</th>
                          <th className="p-3">Mention</th>
                          <th className="p-3">Appréciation Académique</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-150 dark:border-gray-800">
                        {systemConfig.honorsScale.map((h, i) => (
                          <tr key={i} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
                            <td className="p-3 font-mono font-bold text-gray-700 dark:text-gray-300">
                              ≥ {h.min} / {systemConfig.defaultMaxScore}
                            </td>
                            <td className="p-3">
                            ...
                            <td className="p-3 text-gray-500 italic">{h.appreciation}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {/* Official Evaluation Types & Coefs */}
                <div className="space-y-2">
                  <h4 className="text-xs font-black uppercase text-gray-700 dark:text-gray-200 flex items-center gap-1.5">
                    <Sliders size={15} className="text-indigo-500" />
                    Types d'Évaluations & Coefficients Standardisés
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    {systemConfig.evaluationTypes.map((t) => (
                      <div key={t.id} className="p-3 bg-indigo-50/40 dark:bg-indigo-950/20 rounded-xl border border-indigo-100 dark:border-indigo-900/30">
                        <span className="text-xs font-black text-gray-900 dark:text-white block">{t.label}</span>
                        <div className="flex justify-between items-center mt-1 text-[10.5px]">
                          <span className="text-gray-400">Coefficient :</span>
                          <span className="font-black text-indigo-600 dark:text-indigo-400">{t.defaultCoef}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {/* Anti-Fraud Proctor Policies */}
                <div className="p-4 bg-gradient-to-br from-rose-50 to-amber-50 dark:from-rose-950/20 dark:to-amber-950/20 rounded-2xl border border-rose-100 dark:border-rose-900/30 space-y-2">
                  <h4 className="text-xs font-black uppercase text-rose-800 dark:text-rose-300 flex items-center gap-1.5">
                    ...
                    Règles Actives du Chaperon Anti-Fraude
                  </h4>
                  <ul className="text-xs space-y-1.5 text-gray-700 dark:text-gray-300">
                    <li className="flex items-start gap-2">
                      <span className="text-rose-500 font-bold">•</span>
                      <span><strong>Verrouillage temporel :</strong> Aucune note ne peut être modifiée après {systemConfig.antiFraudRules.lockDelayHours} heures sans dérogation administrative.</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-rose-500 font-bold">•</span>
                      <span><strong>Détection de saut suspect :</strong> Tout écart brutal supérieur à ±{systemConfig.antiFraudRules.varianceThreshold} points par rapport à la moyenne historique déclenche une alerte de sécurité.</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-rose-500 font-bold">•</span>
                      <span><strong>Traçabilité immuable :</strong> Chaque rectification exige un motif obligatoire, horodaté et consigné dans le registre d'audit certifié.</span>
                    </li>
                  </ul>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-150 dark:border-gray-700">
                    <span className="text-[10px] font-black uppercase text-gray-400 block mb-1">Diplômes Préparés</span>
                    <div className="flex flex-wrap gap-1">
                      {systemConfig.diplomas.map(d => (
                        <span key={d} className="px-2 py-0.5 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-[10px] font-bold rounded shadow-xs">
                          {d}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-150 dark:border-gray-700">
                    <span className="text-[10px] font-black uppercase text-gray-400 block mb-1">Cycles Pédagogiques</span>
                    <div className="flex flex-wrap gap-1">
                      {systemConfig.cycles.map(c => (
                        <span key={c} className="px-2 py-0.5 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-[10px] font-bold rounded shadow-xs">
                          {c}
                        </span>
                      ))}
                    </div>
              ...
              {/* Footer */}
              <div className="p-4 bg-gray-50 dark:bg-gray-800 border-t border-gray-150 dark:border-gray-700 flex justify-end">
                <button
                  type="button"
                  onClick={() => setShowSystemModal(false)}
                  className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold rounded-xl text-xs uppercase cursor-pointer"
                >
