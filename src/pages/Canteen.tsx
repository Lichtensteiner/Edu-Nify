...
export default function Canteen() {
  ...
  // Mode hors-ligne automatique Gabon-Local-Sync en temps réel
  const [isOnline, setIsOnline] = useState(navigator.onLine);
  const [syncing, setSyncing] = useState(false);
  const [offlineCanteenQueue, setOfflineCanteenQueue] = useState<any[]>(() => {
    try {
      return JSON.parse(localStorage.getItem('canteen_offline_queue') || '[]');
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    localStorage.setItem('canteen_offline_queue', JSON.stringify(offlineCanteenQueue));
  }, [offlineCanteenQueue]);
  ...
  const syncOfflineQueue = async () => {
    if (offlineCanteenQueue.length === 0) return;
    if (!isOnline) {
      setMessagePOS({ text: "⚠️ Impossible de synchroniser en mode hors-ligne.", type: 'warn' });
      return;
    }
    setSyncing(true);
    let successCount = 0;
    const remainingQueue = [...offlineCanteenQueue];

    for (const item of offlineCanteenQueue) {
      try {
        if (item.type === 'consumption') {
          ...
          if (item.menuId) {
            try {
              const menuRef = doc(db, 'canteen_menu', item.menuId);
              await updateDoc(menuRef, {
                portions: Math.max(0, (item.currentMenuPortions || 100) - 1)
              });
            } catch (err) {
              console.warn("Failed to update menu portions during sync:", err);
            }
          }
        }
        successCount++;
        ...
        console.error("Error synchronizing item:", err);
        break;
      }
    }

    setOfflineCanteenQueue(remainingQueue);
    setSyncing(false);
    if (successCount > 0) {
      setMessagePOS({ 
        text: `✓ ${successCount} transactions cantine synchronisées avec succès vers le Cloud Gabon-Nify.`, 
        type: 'success' 
      });
    }
  };
  ...
  useEffect(() => {
    if (isOnline && offlineCanteenQueue.length > 0) {
      syncOfflineQueue();
    }
  }, [isOnline]);
    ...
    try {
      ...
      if (type === 'payant') {
        ...
        if (!isOfflineMode) {
          ...
          selectedStudent.balance = selectedStudent.balance - price;
        }
      }
      ...
      if (!isOfflineMode) {
        ...
        setOfflineCanteenQueue(prev => [...prev, {
          type: 'consumption',
          menuId: currentMenu.id,
          currentMenuPortions: currentMenu.portions,
          data: {
            ...consumptionData,
            offline: true,
      ...
      if (currentMenu.id) {
        if (!isOfflineMode) {
          ...
          currentMenu.portions = Math.max(0, (currentMenu.portions || 100) - 1);
        }
      }
      ...
      if (isOfflineMode) {
        setMessagePOS({ 
          text: `⚡ Enregistré en Mode Hors-ligne (Gabon-Sync) ! ${type === 'inclus' ? 'Repas servi' : 'Repas facturé ' + price + ' FCFA'}. Le ticket est mis en cache locale.`, 
          type: 'success' 
        });
      } else {
  ...
  const handleCheckoutCart = async () => {
    ...
    try {
      ...
      if (!isOfflineMode) {
        ...
        selectedStudent.balance = newBal;
      }
      ...
      for (const item of cartItemsList) {
        ...
        if (matchesStock) {
          if (!isOfflineMode) {
            ...
            matchesStock.quantity = Math.max(0, (matchesStock.quantity || 0) - item.qty);
          }
        }
      }
      ...
      if (isOfflineMode) {
        setMessagePOS({ 
          text: `⚡ Achat de ${totalCost} FCFA enregistré avec succès en Mode Hors-ligne ! (Mis en cache locale Gabon-Sync)`, 
          type: 'success' 
        });
      } else {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      {/* BANNER MODE HORS-LIGNE SELECTION (GABON-LOCAL-SYNC) */}
      <div className={`p-4 rounded-3xl border flex flex-col md:flex-row items-center justify-between gap-4 transition-all ${
        (!isOnline)
          ? 'bg-amber-50 border-amber-200 dark:bg-amber-950/20 dark:border-amber-800/40 text-amber-800 dark:text-amber-300'
          : 'bg-emerald-50/50 border-emerald-100 dark:bg-emerald-950/10 dark:border-emerald-800/20 text-emerald-800 dark:text-emerald-300'
      }`}>
          ...
          <div>
            <div className="text-xs font-black uppercase tracking-wider flex items-center gap-1.5">
              ...
              <span className="underline decoration-2">
                {(!isOnline) ? 'Mode Hors-ligne Sécurisé (Cache local Gabon-Sync)' : 'Connecté au serveur Cloud Gabon-Nify'}
              </span>
            </div>
            <p className="text-[10px] opacity-85 mt-0.5 font-medium leading-normal">
              {(!isOnline)
                ? `Les encaissements et pointages cantine sont temporairement stockés dans le navigateur. (${offlineCanteenQueue.length} repas en attente).`
                : "Toutes les transactions sont synchronisées en temps réel vers la base de données de l'établissement."
              }
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
          {offlineCanteenQueue.length > 0 && (
            <button
              onClick={syncOfflineQueue}
              disabled={syncing || !isOnline}
              className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-400 text-white font-black text-[10px] uppercase tracking-wider rounded-xl shadow transition-all flex items-center gap-1 cursor-pointer"
            >
              {syncing ? 'Synchro...' : `Synchroniser (${offlineCanteenQueue.length})`}
            </button>
          )}
        </div>
      </div>
