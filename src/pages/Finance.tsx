...
2. "audit" : Une note technique d'audit interne pour le comptable de l'établissement. Elle valide les comptes d'imputation SYSCOHADA (ex : Débit 521000 et Crédit 701000, etc.), donne un diagnostic santé ou une évaluation rapide des anomalies/doublons d'écriture de trésorerie de l'élève. Doit être court et exploitable.`;
  ...
  const downloadInvoiceAsFile = async (payment: Payment, notesOverrider?: string, auditOverrider?: string) => {
    const dateString = payment.date?.seconds 
      ? new Date(payment.date.seconds * 1000).toLocaleDateString('fr-FR', { dateStyle: 'long' })
      : payment.date?.toDate 
        ? payment.date.toDate().toLocaleDateString('fr-FR', { dateStyle: 'long' }) 
        : new Date(payment.date || Date.now()).toLocaleDateString('fr-FR', { dateStyle: 'long' });
    ...
    // construct receipt object matching pdfGenerator expectations
    const receiptObj = {
      id: payment.id,
      reference: finalReference,
      date: dateString,
      studentName: finalStudentName,
      etablissement: (payment as any).etablissement || currentEstablishment?.id || 'EDU-001',
      type: payment.type,
      method: payment.method,
      amount: finalAmount,
      notes: finalNotes,
      recordedByName: payment.recordedByName || 'Comptable'
    };

    try {
      await generateReceiptPDF(receiptObj, currentEstablishment);
    } catch (error) {
  ...
  const handleExportSage = () => {
    ...
    accountingEntries.forEach(ent => {
      ...
      rows.push([
        ...
        ent.amount,             // Debit
        0,                      // Credit
        ent.hash && ent.hash.includes('-') ? ent.hash.split('-')[1] : (ent.hash || 'N/A')  // Signature
      ]);
      ...
      rows.push([
        ...
        0,
        ent.amount,
        ent.hash && ent.hash.includes('-') ? ent.hash.split('-')[1] : (ent.hash || 'N/A')
      ]);
    });
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md overflow-y-auto">
            ...
            >
              ...
              <div className="w-full md:w-2/3 p-6 bg-slate-50 dark:bg-slate-900 border-b md:border-b-0 md:border-r border-gray-100 dark:border-gray-700 overflow-y-auto flex flex-col items-center">
                ...
                <div id="school-invoice-paper" className="w-full max-w-xl bg-white text-black p-4 sm:p-8 rounded-2xl shadow-sm border border-gray-200/60 font-sans relative overflow-hidden">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start border-b-2 border-slate-900 pb-5 gap-4">
                    ...
                    <div className="text-left sm:text-right w-full sm:w-auto">
                      ...
                      <div className="flex items-center text-xs justify-start sm:justify-end font-black text-slate-800 gap-1">
                        <span>Réf:</span>
                        <input
                          type="text"
                          value={receiptReference}
                          onChange={(e) => setReceiptReference(e.target.value)}
                          className="font-black text-slate-800 bg-transparent border-b border-dashed border-gray-300 hover:border-indigo-400 focus:border-indigo-500 focus:bg-indigo-50/50 outline-none text-left sm:text-right w-24 rounded px-1 py-0.5 transition-all"
                        />
                      </div>
                  ...
                  {/* Dynamic Items Table with horizontal scroll container */}
                  <div className="w-full overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0 scrollbar-thin mb-6">
                    <table className="w-full text-left text-xs text-gray-600 border-collapse min-w-[380px] sm:min-w-0">
                      <thead>
                        <tr className="border-b border-slate-900 font-black text-slate-800">
                          <th className="py-2 pr-2">Description du versement</th>
                          <th className="py-2 px-2 uppercase">Rubrique</th>
                      ...
                      <tbody className="divide-y divide-gray-150">
                        <tr>
                          <td className="py-3 pr-2 font-medium text-slate-900 text-[11px] max-w-[150px] sm:max-w-none break-words">
                          ...
                          <td className="py-3 px-2 font-semibold text-indigo-800 uppercase tracking-wider text-[10px]">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center my-6 pt-4 border-t border-dashed border-gray-300 text-[10px] gap-6">
                    <div>
                    ...
                    <div className="text-left sm:text-right w-full sm:w-auto">
                      <p className="font-extrabold uppercase text-gray-400 mb-2 sm:mb-6">Signature Reçue</p>
                      <p className="font-bold text-slate-900 underline">Le Directeur des Finances</p>
                    </div>
                  </div>

                  {/* Security watermark hash confirmation */}
                  <div className="text-[8px] text-gray-400 flex flex-col sm:flex-row items-stretch sm:items-center justify-between font-mono bg-slate-50 p-2.5 rounded-lg border border-gray-100 uppercase mt-4 gap-2">
                    <span className="truncate">Edu-Nify Anti-Falsification ID : {selectedInvoice.id}</span>
                    <span className="text-emerald-600 font-bold">SHA-HASH: {generateTransactionHash(selectedInvoice.id, '521000', '701000', receiptAmount, '2026').substring(0, 16)}...</span>
                  </div>
                </div>
              </div>
