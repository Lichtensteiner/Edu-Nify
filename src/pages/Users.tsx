...
import { sendEmailNotification, createNotification } from '../services/NotificationService';
...
export default function Users() {
  ...
  const cleanStringForPseudo = (str: string): string => {
    if (!str) return '';
    return str
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "") // remove accents
      .replace(/[^a-z0-9_.-]/g, "") // keep only letters, digits, dots, hyphens, underscores
      .trim();
  };
  ...
  const [usePseudoAccount, setUsePseudoAccount] = useState(false);
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    // Support pseudo accounts for student role (6ème-3ème)
    let finalEmail = newUser.email.trim();
    let calculatedPseudo = '';
    
    if (newUser.role === 'élève' && (usePseudoAccount || !finalEmail)) {
      const cleanPrenom = cleanStringForPseudo(newUser.prenom);
      const cleanNom = cleanStringForPseudo(newUser.nom);
      if (!cleanPrenom || !cleanNom) {
        setError("Veuillez renseigner le Nom et le Prénom de l'élève pour pouvoir générer son Pseudo.");
        setActionLoading(false);
        return;
      }
      calculatedPseudo = cleanPrenom + '.' + cleanNom;
      if (newUser.matricule) {
        calculatedPseudo += '.' + cleanStringForPseudo(newUser.matricule).toLowerCase().slice(-4);
      } else {
        calculatedPseudo += '.' + Math.floor(100 + Math.random() * 900);
      }
      finalEmail = `${calculatedPseudo}@edunify.internal`;
    } else if (!finalEmail) {
      setError("Une adresse e-mail est requise pour ce profil.");
      setActionLoading(false);
      return;
    }

    // Real-time email domain and configuration check
    if (!finalEmail.endsWith('@edunify.internal')) {
      try {
        ...
        if (!verifyRes.ok || !verifyData.valid) {
          setError(verifyData.error || "L'adresse e-mail saisie n'est pas valide ou n'existe pas.");
          setActionLoading(false);
          return;
        }
      } catch (verifyErr: any) {
        console.warn("Could not perform real-time email verification via API, continuing:", verifyErr);
      }
    }
    ...
    try {
      ...
      await setDoc(doc(db, 'users', finalUid), {
        ...
        children_ids: finalRole === 'parent' ? (newUser.children_ids || []) : null
      }, { merge: true });

      // Get the establishment name
      const targetEstId = isSuperAdmin ? (newUser.etablissement || currentEstablishment?.id || 'EDU-001') : (currentEstablishment?.id || 'EDU-001');
      const targetEst = establishments?.find((e: any) => e.id === targetEstId);
      const estName = targetEst ? targetEst.nom : (currentEstablishment?.nom || "Edu-Nify");

      // Send real-time email notification
      try {
        await sendEmailNotification(newUser.email, newUser.prenom, newUser.nom, finalRole, newUser.password, estName);
      } catch (emailErr) {
        console.warn("Could not dispatch real-time email notification:", emailErr);
      }
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(userRef, {
        ...
        children_ids: finalRole === 'parent' ? (editUser.children_ids || []) : null
      });
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1 items-start">
                            <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${
                              ...
                              user.role === 'parent' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
            ...
            <div className="flex flex-wrap items-center gap-2.5 w-full sm:w-auto">
              <div className="flex items-center gap-1.5 min-w-[150px]">
                ...
                >
                  ...
                  <option value="parent">{tData('parent')}</option>
                          ...
                          <div>
                            ...
                            {user.pseudo ? (
                              <div className="flex items-center gap-1.5 flex-wrap">
                                <span className="inline-block px-1.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-700 dark:bg-emerald-950/20 dark:text-emerald-400">@{user.pseudo}</span>
                                <span className="text-[10px] text-gray-400 font-medium italic lowercase">({user.email})</span>
                              </div>
                            ) : (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1 items-start">
                          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                            ...
                            user.role === 'parent' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' :
                  ...
                  <div className="relative group overflow-hidden rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm mb-6">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-6 p-6 bg-white dark:bg-gray-800 pt-10 relative">
                      ...
                      <div className="text-center md:text-left mt-8 md:mt-0 md:pl-28">
                        ...
                        {viewUser.pseudo && (
                          <div className="flex flex-wrap justify-center md:justify-start gap-2 mt-2">
                            <span className="px-3 py-1 bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 rounded-full text-xs font-black uppercase tracking-wider">
                  ...
                  {viewUser.role === 'parent' && (
                    <div className="pt-4 border-t border-gray-100 dark:border-gray-700 space-y-4">
                      <h5 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2">
                        ...
                        Élèves rattachés (Enfants)
                      </h5>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {viewUser.children_ids && viewUser.children_ids.length > 0 ? (
                          viewUser.children_ids.map((childId: string) => {
                            const child = users.find(u => u.id === childId);
                            if (!child) return null;
                            return (
                              <div key={childId} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl">
                                {child.photo ? (
                                  <img src={child.photo} alt="" className="w-10 h-10 rounded-full object-cover" referrerPolicy="no-referrer" />
                                ) : (
                                  <div className="w-10 h-10 rounded-full bg-indigo-100 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-sm">
                                    {child.prenom?.[0]}
                                  </div>
                                )}
                                <div className="min-w-0 flex-1">
                                  <p className="text-xs font-bold text-gray-900 dark:text-white truncate">{child.prenom} {child.nom}</p>
                                  <p className="text-[10px] text-indigo-650 dark:text-indigo-400 font-bold truncate">Classe: {child.classe}</p>
                                </div>
                              </div>
                            );
                          })
                        ) : (
                          <p className="text-xs text-gray-400 italic col-span-full">Aucun élève lié à ce parent.</p>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      >
                        ...
                        {(isSuperAdmin || currentUser?.role === 'admin') && (
                          <option value="parent">Parent d'Élève ({tData('parent')})</option>
                        )}
                        {isSuperAdmin && (
                    ...
                    {editUser.role === 'élève' && (
                      <div className="space-y-4 col-span-full">
                        ...
                        <div>
                          <label className="block text-xs font-semibold text-indigo-700 dark:text-indigo-400 mb-1.5 ml-1 uppercase">Pseudo de Connexion (Optionnel - Pour se connecter sans e-mail)</label>
                          <input
                            type="text"
                            value={editUser.pseudo || ''}
                            onChange={(e) => setEditUser({...editUser, pseudo: cleanStringForPseudo(e.target.value)})}
                            placeholder="Ex: prenom.nom"
                            className="w-full px-4 py-2.5 bg-indigo-50/20 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/50 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-sm font-semibold text-indigo-800 dark:text-indigo-300"
                          />
                          <p className="text-[10px] text-gray-400 mt-1 pl-1">
                            Si renseigné, cet élève pourra se connecter avec ce Pseudo et son mot de passe au lieu de son adresse e-mail.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    {editUser.role === 'parent' && (
                      <div className="col-span-1 md:col-span-2 space-y-4 animate-fade-in border-t border-gray-100 dark:border-gray-800 pt-4">
                        <label className="block text-xs font-black text-gray-500 mb-2 ml-1 uppercase flex items-center gap-2">
                          <span>🔗 Élèves rattachés (Enfants)</span>
                        </label>
                        <div className="space-y-2">
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full pl-10 pr-4 py-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-sm"
                              value={studentSearchTerm}
                              onChange={(e) => setStudentSearchTerm(e.target.value)}
                            />
                          </div>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-52 overflow-y-auto p-3 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900/50">
                            {(() => {
                              const matchedStudents = users.filter(u => {
                                  if (u.role !== 'élève') return false;
                                  const search = studentSearchTerm.toLowerCase();
                                  const fullName = `${u.prenom} ${u.nom}`.toLowerCase();
                                  const classe = (u.classe || '').toLowerCase();
                                  const matricule = (u.matricule || '').toLowerCase();
                                  return fullName.includes(search) || classe.includes(search) || matricule.includes(search);
                              });

                              if (matchedStudents.length === 0) {
                                return (
                                  <p className="text-xs text-gray-400 p-4 italic text-center col-span-full">
                                    Aucun élève trouvé.
                                  </p>
                                );
                              }
                              ...
                              return matchedStudents.map(student => {
                                const isChecked = (editUser.children_ids || []).includes(student.id);
                                return (
                                  <label key={student.id} className="flex items-center gap-3 p-2 bg-white dark:bg-gray-850 rounded-lg cursor-pointer transition-all border border-gray-150 dark:border-gray-800 hover:border-indigo-200 select-none">
                                    <input
                                      type="checkbox"
                                      checked={isChecked}
                                      onChange={(e) => {
                                        const currentChildren = editUser.children_ids || [];
                                        if (e.target.checked) {
                                          setEditUser({...editUser, children_ids: [...currentChildren, student.id]});
                                        } else {
                                          setEditUser({...editUser, children_ids: currentChildren.filter((id: string) => id !== student.id)});
                                        }
                                      }}
                                      className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 w-4 h-4 shrink-0"
                                    />
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <p className="text-[10px] text-gray-550 dark:text-gray-400 truncate">
                                        Classe: <strong className="text-indigo-600 dark:text-indigo-400">{student.classe}</strong> | Matricule: <strong className="font-mono">{student.matricule || 'N/A'}</strong>
                                      </p>
                                    </div>
                                  </label>
                                );
              ...
              <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    {newUser.role === 'élève' && (
                      <div className="p-3.5 rounded-xl bg-indigo-50/40 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/50 space-y-2 mb-2">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={usePseudoAccount}
                            onChange={(e) => {
                              setUsePseudoAccount(e.target.checked);
                              if (e.target.checked) {
                          ...
                          <span className="text-xs font-bold text-indigo-900 dark:text-indigo-300">
                            Activer le compte par Pseudo (Pas d'e-mail requis pour l'élève)
                          </span>
                        </label>
                        <p className="text-[10px] text-gray-500 leading-normal pl-5">
                          Recommandé pour les élèves du Collège (6ème à 3ème). L'élève se connectera en saisissant son Pseudo et son mot de passe. Les parents pourront lier le compte pour suivre sa traçabilité en temps réel.
                        </p>
                        {usePseudoAccount && newUser.prenom && newUser.nom && (
                          <div className="mt-2 p-2 bg-white dark:bg-gray-900 rounded-lg border border-indigo-100 dark:border-indigo-800/50 text-[11px] font-medium text-gray-700 dark:text-gray-300 pl-5">
                            <span className="font-bold text-emerald-600 dark:text-emerald-400">⚡ Identifiant de connexion :</span> @{cleanStringForPseudo(newUser.prenom)}.{cleanStringForPseudo(newUser.nom)}{newUser.matricule ? `.${cleanStringForPseudo(newUser.matricule).toLowerCase().slice(-4)}` : ''}
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {!usePseudoAccount ? (
                      ...
                      <div className="opacity-70">
                        <label className="block text-xs font-medium text-gray-450 mb-1.5 ml-1 uppercase">Adresse E-mail Virtuelle Générée</label>
                        <div className="relative">
                          <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                          <input
                            type="text"
                            disabled
                            value={`${cleanStringForPseudo(newUser.prenom)}.${cleanStringForPseudo(newUser.nom)}${newUser.matricule ? `.${cleanStringForPseudo(newUser.matricule).toLowerCase().slice(-4)}` : ''}@edunify.internal`}
                            className="w-full pl-10 pr-4 py-2.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-750 rounded-xl outline-none text-sm font-mono text-gray-500"
                          />
                        </div>
                      </div>
                    )}
                ...
                <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      ...
                      {newUser.role === 'parent' && (
                        <div className="space-y-4 animate-fade-in border-t border-gray-100 dark:border-gray-800 pt-4">
                          <label className="block text-xs font-black text-gray-500 mb-2 ml-1 uppercase flex items-center gap-2">
                            <span>🔗 Élèves rattachés (Enfants)</span>
                          </label>
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-52 overflow-y-auto p-3 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900/50">
                              {(() => {
                                ...
                                return matchedStudents.map(student => {
                                  const isChecked = (newUser.children_ids || []).includes(student.id);
                                  return (
                                    <label key={student.id} className="flex items-center gap-3 p-2 bg-white dark:bg-gray-850 rounded-lg cursor-pointer transition-all border border-gray-150 dark:border-gray-800 hover:border-indigo-200 select-none">
                                      <input
                                        type="checkbox"
                                        checked={isChecked}
                                        onChange={(e) => {
                                          const currentChildren = newUser.children_ids || [];
                                          if (e.target.checked) {
                                            setNewUser({...newUser, children_ids: [...currentChildren, student.id]});
                                          } else {
                                            setNewUser({...newUser, children_ids: currentChildren.filter((id: string) => id !== student.id)});
                                          }
                                        }}
                                        className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 w-4 h-4 shrink-0"
                                      />
